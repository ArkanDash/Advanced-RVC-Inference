...
from arvc.rvc.inference.tts import TTS
from arvc.utils.process import process_input
from arvc.rvc.inference.inference import convert_tts
from arvc.utils.utils import google_translate
from arvc.rvc.inference.presets import save_presets, load_presets
from arvc.utils.feedback import visible, change_f0_choices, unlock_f0, hoplength_show, change_models_choices, get_index, index_strength_show, change_embedders_mode, change_tts_voice_choices, shutil_move, change_preset_choices
...
def convert_tts_tab():
    ...
    with gr.Row():
        with gr.Column():
            txt_input = gr.File(label=translations["drop_text"], file_types=[".txt", ".srt", ".docx"], visible=False)  
    ...
    with gr.Row():    
        with gr.Column():
            with gr.Accordion(translations["output_path"], open=False):
                ...
                output_audio0 = gr.Textbox(label=translations["output_tts"], value="arvc/assets/audios/tts/tts.wav", placeholder="audios/tts.wav", info=translations["tts_output"], interactive=True)
                output_audio1 = gr.Textbox(label=translations["output_tts_convert"], value="arvc/assets/audios/rvc/tts-convert.wav", placeholder="audios/tts-convert.wav", info=translations["tts_output"], interactive=True)
    with gr.Row():    
        with gr.Column():
            with gr.Accordion(translations["setting"], open=False):
                with gr.Accordion(translations["f0_method"], open=False):
                    with gr.Group():
                        ...
                        hybrid_method0 = gr.Dropdown(label=translations["f0_method_hybrid"], info=translations["f0_method_hybrid_info"], choices=hybrid_f0_method, value=hybrid_f0_method[0], interactive=True, allow_custom_value=True, visible=False)
