...
from arvc.downloader import huggingface, gdown, meganz, mediafire, pixeldrain
from arvc.utils.process import move_files_from_directory, fetch_pretrained_data, extract_name_model
...
def download_pretrained_model(choices, model, sample_rate):
    ...
    if choices == translations.get("list_model", "List Model"):
        ...
        alt_base_url = configs.get("pretrained_custom_url", "https://huggingface.co/buckets/R-Kentaren/Ultimate-RVC-Models/resolve/pretrained_custom/")
        if not alt_base_url:
