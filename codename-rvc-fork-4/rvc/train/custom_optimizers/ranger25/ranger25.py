...
# Integration of RAdam as an extra core by: Codename;0
...
class ranger25(TO.Optimizer):
    def __init__(
        ...
        if self.use_radam and self.use_madgrad:
            raise ValueError("Please choose only one core: either madgrad or RAdam.")
        if self.use_radam:
            self.core_engine = "RAdam"
        elif self.use_madgrad:
    ...
    def show_schedule(self):
        ...
        startlr = x[0]
        plt.title(
            f"ranger25 learning rate schedule\nStart={startlr:.2E}\nMax ={maxlr:.2E}\n,Min={minlr:.2E}\n"
        )
        plt.show()

    def show_settings(self):
        print(f"ranger25 optimizer ready with following settings:\n")
    ...
    def warmup_dampening(self, lr, step):
        ...
        if step > warmup:
            if not self.warmup_complete:
                ...
                print(f"\n** ranger25 update = Warmup complete - lr set to {lr}\n")
            return lr
    ...
    def get_warm_down(self, lr, iteration):
        ...
        if iteration > self.start_warm_down - 1:
            ...
            if not self.warmdown_displayed:
                print(
                    f"\n** ranger25 update: Warmdown starting now.  Current iteration = {iteration}....\n"
                )
        ...
        if self.use_radam:
            for group in self.param_groups:
                ...
                for p in group["params"]:
                    ...
                    # Apply gradient centralization/normalization if enabled
                    if self.use_gc:
                    ...
                    # Apply warmup/warmdown adjustments if enabled
                    effective_lr = lr
                    if self.use_warmup and not self.warmup_complete:
                        effective_lr = self.warmup_dampening(lr, step)
                    if self.warmdown_active:
                        effective_lr = self.get_warm_down(effective_lr, step)

                    # RAdam buffered computations (buffer is created per group)
                    if "buffer" not in group:
                    ...
                    # Update parameters using RAdam update rule
                    if N_sma >= 5:
                        ...
                        p_data_fp32.addcdiv_(exp_avg, denom, value=-step_size * effective_lr)
                        # p_data_fp32.addcdiv_(-step_size * effective_lr, exp_avg, denom) # ~ deprecated
                        p.data.copy_(p_data_fp32)
                    elif step_size > 0:
                        if group["weight_decay"] != 0:
                            p_data_fp32.add_(-group["weight_decay"] * effective_lr, p_data_fp32)
                        p_data_fp32.add_(-step_size * effective_lr, exp_avg)
                        p.data.copy_(p_data_fp32)
