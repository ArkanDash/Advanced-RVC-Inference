...
def extract_features(model, feats, version, device="cpu", mix=False, mix_layers=9, mix_ratio=0.5):
    ...
    with torch.no_grad():
        ...
        # BUG FIX: Save original raw-audio input before overwriting `feats` with
        # projected features. The original code passed the PROJECTED features as
        # `source` to the second extract_features call (which expects raw audio),
        # producing garbage features and corrupting training when embedders_mix=True.
        padding_mask = torch.BoolTensor(feats.shape).fill_(False).to(device)
        logits = model.extract_features(source=feats, padding_mask=padding_mask, output_layer=output_layer)
        feats_out = model.final_proj(logits[0]) if version == "v1" else logits[0]

        # VRVC: embedder layer mixing — blend features from two layers.
        # Use the ORIGINAL raw audio input (`feats`), not the projected output.
        if mix and len(logits) > 1:
            mix_layer_feats = model.extract_features(source=feats, padding_mask=padding_mask, output_layer=mix_layers)
            mix_feats = mix_layer_feats[0] if version != "v1" else model.final_proj(mix_layer_feats[0])
            feats_out = feats_out * mix_ratio + mix_feats * (1 - mix_ratio)
