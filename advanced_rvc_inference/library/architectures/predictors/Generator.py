...
from advanced_rvc_inference.library.architectures.predictors.CREPE.filter import mean, median
from advanced_rvc_inference.library.architectures.predictors.WORLD.SWIPE import swipe, stonemask
from advanced_rvc_inference.utils.variables import config, configs, logger, translations
from advanced_rvc_inference.library.architectures.utils import autotune_f0, proposal_f0_up_key, circular_write
...
class Generator:
    ...
    def get_f0_mangio_crepe(self, x, p_len, model="full"):
        if not hasattr(self, "mangio_crepe"):
            from advanced_rvc_inference.library.architectures.predictors.CREPE.CREPE import CREPE
    ...
    def get_f0_fcpe(self, x, p_len, legacy=False, previous=False, filter_radius=3):
        if not hasattr(self, "fcpe"): 
            from advanced_rvc_inference.library.architectures.predictors.FCPE.FCPE import FCPE
    ...
    def get_f0_rmvpe(self, x, p_len, clipping=False, filter_radius=3):
        if not hasattr(self, "rmvpe"): 
            from advanced_rvc_inference.library.architectures.predictors.RMVPE.RMVPE import RMVPE
    ...
    def get_f0_pyworld(self, x, p_len, filter_radius, model="harvest"):
        if not hasattr(self, "pw"): 
            from advanced_rvc_inference.library.architectures.predictors.WORLD.WORLD import PYWORLD
    ...
    def get_f0_penn(self, x, p_len, filter_radius=3):
        if not hasattr(self, "penn"):
            from advanced_rvc_inference.library.architectures.predictors.PENN.PENN import PENN
    ...
    def get_f0_djcm(self, x, p_len, clipping=False, filter_radius=3):
        if not hasattr(self, "djcm"): 
            from advanced_rvc_inference.library.architectures.predictors.DJCM.DJCM import DJCM
    ...
    def get_f0_swift(self, x, p_len, filter_radius=3):
        if not hasattr(self, "swift"): 
            from advanced_rvc_inference.library.architectures.predictors.SWIFT.SWIFT import SWIFT
    ...
    def get_f0_pesto(self, x, p_len):
        if not hasattr(self, "pesto"):
            from advanced_rvc_inference.library.architectures.predictors.PESTO.PESTO import PESTO
