...
from advanced_rvc_inference.library.architectures.algorithm.encoders import Encoder
...
class ResidualCouplingTransformersLayer2(nn.Module):  # VITS2
    def __init__(
        ...
        assert channels % 2 == 0, "channels should be divisible by 2"
        super(ResidualCouplingTransformersLayer2, self).__init__()
        self.channels = channels
        self.hidden_channels = hidden_channels
        self.kernel_size = kernel_size
        self.dilation_rate = dilation_rate
...
#        return x
        if not reverse:
            ...
            for flow in reversed(self.flows):
                out = flow(x, x_mask, g=g, reverse=reverse)
