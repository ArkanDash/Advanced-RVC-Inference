...
A flow-based vocoder using invertible 1x1 convolutions and
WaveNet-like layers with gated activations.
Simple but effective architecture for high-quality audio synthesis.

Based on: WaveGlow: A Flow-based Generative Network for Speech Synthesis
(Prenger, Valle, Catanzaro, 2019)

Adapted for RVC compatibility with the standard generator interface.
...
Args:
    ...
    resblock_kernel_sizes: Tuple of kernel sizes for WN blocks.
    resblock_dilations: Tuple of dilation tuples for WN blocks.
    gin_channels: Speaker embedding channels.
    sr/sample_rate: Sample rate.
    harmonic_num: Number of harmonics (used for source excitation).
...
class WNLayer(nn.Module):
    """WaveNet-like dilated convolution layer with gated activation."""
    ...
    def forward(self, x):
        ...
        return (residual + x) / math.sqrt(2), skip
...
class WaveNetBlock(nn.Module):
    """Stack of WaveNet layers with dilations."""

    def __init__(self, channels, kernel_size=3, dilations=(1, 2, 4, 8)):
        super().__init__()
        self.layers = nn.ModuleList()
        for d in dilations:
            self.layers.append(WNLayer(channels, kernel_size, d))
...
class Invertible1x1Conv(nn.Module):
    """Invertible 1x1 convolution (simplified for forward-only inference)."""
...
class WaveGlowGenerator(nn.Module):
    """
    WaveGlow Generator.

    Flow-based vocoder with:
    - Invertible 1x1 convolutions for channel mixing
    - WaveNet-like layers with gated activations
    - Standard upsampling path for mel conditioning
    ...
    Args:
        ...
        resblock_kernel_sizes: Kernel sizes for WaveNet blocks.
        resblock_dilations: Dilation configs for WaveNet blocks.
        gin_channels: Speaker embedding channels.
        sr/sample_rate: Sample rate.
    ...
    def __init__(
        ...
        resblock_dilations=((1, 2, 4, 8), (1, 2, 4, 8), (1, 2, 4, 8)),
        gin_channels=256,
        sr=48000,
        sample_rate=None,
        harmonic_num=0,
        ...
        # WaveNet blocks with invertible 1x1 convolutions
        self.wn_blocks = nn.ModuleList()
        self.inv_convs = nn.ModuleList()
        for i in range(len(self.ups)):
            ch = channels[i]
            self.wn_blocks.append(
                nn.ModuleList([
                    WaveNetBlock(ch, kernel_size=k, dilations=d)
                    for k, d in zip(resblock_kernel_sizes, resblock_dilations)
                ])
            )
            self.inv_convs.append(Invertible1x1Conv(ch))
    ...
    def forward(self, x, f0, g=None):
        ...
        for i, (ups, noise_conv, inv_conv) in enumerate(
            zip(self.ups, self.noise_convs, self.inv_convs)
        ):
            x = F.leaky_relu(x, LRELU_SLOPE)

            if self.training and self.checkpointing:
                x = checkpoint(ups, x, use_reentrant=False) + noise_conv(har_source)
                x = checkpoint(inv_conv, x, use_reentrant=False)
                skip_sum = 0
                for wn in self.wn_blocks[i]:
                    _, skip = checkpoint(wn, x, use_reentrant=False)
                    skip_sum = skip_sum + skip
                x = x + skip_sum / self.num_kernels
            else:
