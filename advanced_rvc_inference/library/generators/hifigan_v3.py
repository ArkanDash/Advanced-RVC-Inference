"""
HiFi-GAN v3 vocoder implementation.

An improved HiFi-GAN with SnakeBeta activations (inspired by BigVGAN)
and enhanced residual blocks with wider dilated convolutions.
Simpler than BigVGAN while retaining much of its quality.
...
class HiFiGANV3ResBlock(nn.Module):
    """Enhanced residual block with SnakeBeta activation and multi-dilation."""
...
class SineGen(nn.Module):
    """Sine wave generator for f0-based harmonic excitation."""
...
class HiFiGANV3Generator(nn.Module):
    """
    HiFi-GAN v3 Generator.

    An improved HiFi-GAN with:
    - SnakeBeta activations (like BigVGAN but simpler, no anti-aliasing)
    - Better residual blocks with dilated convolutions
    - Multi-period style feature fusion

    Args:
        in_channel: Input mel channels.
        upsample_initial_channel: Initial channel dimension for upsampling.
        ...
        resblock_dilations: Dilation configurations for residual blocks.
        gin_channels: Speaker embedding channels.
        sr/sample_rate: Sample rate.
    ...
    def __init__(
        ...
        # Residual blocks with SnakeBeta
        self.resblocks = nn.ModuleList()
        for i in range(len(self.ups)):
            ch = channels[i]
            self.resblocks.append(
                nn.ModuleList([
                    HiFiGANV3ResBlock(ch, kernel_size=k, dilations=d)
                    for k, d in zip(resblock_kernel_sizes, resblock_dilations)
                ])
            )

        # Post-processing with SnakeBeta
        self.act_post = SnakeBeta(channels[-1])
        self.conv_post = weight_norm(nn.Conv1d(channels[-1], 1, kernel_size=7, padding=3))
