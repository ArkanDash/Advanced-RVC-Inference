...
Launches the Gradio web interface for voice conversion, training, and real-time processing.
"""
...
def setup_environment():
    ...
    os.makedirs("advanced_rvc_inference/assets/audios", exist_ok=True)
    os.makedirs("advanced_rvc_inference/assets/dataset", exist_ok=True)
    os.makedirs("advanced_rvc_inference/assets/audios/uvr", exist_ok=True)
    os.makedirs("advanced_rvc_inference/assets/audios/rvc", exist_ok=True)
...
def launch(
    ...
    try:
        ...
        if "Address already in use" in str(e):
            ...
            logger.info("Try using a different port with: --port 7861")
        else:
            logger.error(f"Failed to start server: {e}")
        return 1
    except Exception as e:
        logger.error(f"Failed to launch web interface: {e}")
        logger.info("Try running with: --share=False")
        return 1
