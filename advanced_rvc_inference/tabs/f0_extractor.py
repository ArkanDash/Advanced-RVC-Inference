...
class AdvancedF0Extractor:
        ...
        with torch.no_grad():
            ...
            if torch.cuda.is_available() and self.device != "cpu":
                audio_tensor = torch.from_numpy(
                    audio).float().cuda().unsqueeze(0)
            else:
                audio_tensor = torch.from_numpy(
                    audio).float().cpu().unsqueeze(0)
    ...
    def extract_hybrid_f0(
        ...
        for method in methods:
            ...
            if "crepe" in method:
                f0 = self.extract_f0_crepe(
                    audio, sr, method, hop_length, f0_min, f0_max)
            elif method == "fcpe":
                f0 = self.extract_f0_fcpe(
                    audio, sr, hop_length, f0_min, f0_max)
            elif method in ["rmvpe", "dio", "harvest", "pyin", "pm"]:
                f0 = self.extract_f0_traditional(
                    audio, sr, method, hop_length, f0_min, f0_max)
            else:
                f0 = self.extract_f0_traditional(
                    audio, sr, "rmvpe", hop_length, f0_min, f0_max)
    ...
    def extract_f0(self,
                   ...
                   hop_length: int = 128,
                   f0_min: float = 50,
                   f0_max: float = 1100) -> tuple[np.ndarray,
                                                  str]:
        ...
        try:
            ...
            if method.startswith("hybrid["):
                ...
                f0 = self.extract_hybrid_f0(
                    audio, sr, methods_list, hop_length, f0_min, f0_max)
            else:
...
def f0_extractor_tab():
    with gr.Row():
        with gr.Column():
            ...
            gr.Markdown(
                i18n("Extract pitch contours from audio using various methods."))
