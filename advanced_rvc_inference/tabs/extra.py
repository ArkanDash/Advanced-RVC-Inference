...
def extra_tab():
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Tab("Model Analysis"):
                    model_analysis_input = gr.File(
                        label=i18n("Model File"), file_types=[
                            ".pth", ".onnx"])
                    analyze_model_btn = gr.Button(
                        i18n("Analyze Model"), variant="primary")

                with gr.Tab("Batch Processing"):
                    batch_input_folder = gr.Textbox(
                        label=i18n("Input Folder Path"))
                    batch_output_folder = gr.Textbox(
                        label=i18n("Output Folder Path"))
                    batch_convert_btn = gr.Button(
                        i18n("Batch Convert"), variant="primary")

                with gr.Tab("Model Conversion"):
                    convert_input = gr.File(
                        label=i18n("Input Model"), file_types=[".pth"])
                    ...
                    convert_btn = gr.Button(
                        i18n("Convert Model"), variant="primary")

            with gr.Column():
                analysis_output = gr.JSON(label=i18n("Model Analysis"))
                batch_status = gr.Textbox(
                    label=i18n("Batch Status"), interactive=False)
                conversion_status = gr.Textbox(
                    label=i18n("Conversion Status"), interactive=False)
    ...
    def batch_process(input_folder, output_folder):
        ...
        return f"{
            i18n('Batch processing completed for folder:')} {input_folder}"
    ...
    def convert_model(model_file, target_format):
        ...
        return f"{
            i18n('Model conversion to')} {target_format} {
            i18n('completed')}"
