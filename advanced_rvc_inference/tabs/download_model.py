...
def fetch_models_data(search):
    ...
    while True:
        try:
            ...
            url = codecs.decode(
                "uggcf://ibvpr-zbqryf.pbz/srgpu_qngn.cuc", "rot13")
            response = requests.post(
                url, data={"page": page, "search": search})
    ...
    if len(tables) == 0:
        ...
        for table in tables:
            for row in BeautifulSoup(table, "html.parser").select("tr"):
                ...
                url_tag = row.find(
                    "a", {
                        "class": "btn btn-sm fw-bold btn-light ms-0 p-1 ps-2 pe-2"})

                if name_tag and url_tag:
                    url = url_tag["href"].replace(
                        "https://easyaivoice.com/run?url=", "")
                    if "huggingface" in url:
                        # Simple model name replacement
                        def replace_modelname(model):
                            # This would normally replace certain patterns in
                            # model names
                            return model
        ...
        with gr.Tab("Online Model Hub"):
            ...
            with gr.Row():
                ...
                download_selected = gr.Button(
                    "Download Selected Model", variant="primary")
        ...
        with gr.Tab("Voice Models"):
            ...
            with gr.Row():
                ...
                search_button = gr.Button(
                    "Search Pre-trained Models", variant="secondary")
