...
def plugins_tab():
    with gr.Row():
        with gr.Column():
            ...
            gr.Markdown(
                i18n("Manage and configure plugins for extended functionality."))
            ...
            with plugin_actions:
                enable_plugin = gr.Button(
                    i18n("Enable Selected"), variant="primary")
                disable_plugin = gr.Button(
                    i18n("Disable Selected"), variant="secondary")
                install_plugin = gr.Button(
                    i18n("Install Plugin"), variant="primary")

        with gr.Column():
            gr.Markdown(i18n("### Plugin Configuration"))
            selected_plugin_config = gr.JSON(
                label=i18n("Plugin Configuration"))
    ...
    def toggle_plugin(plugin_name, action):
        ...
        return f"{action} action performed on {plugin_name}", f"{plugin_name} {
            action.lower()}d successfully"
