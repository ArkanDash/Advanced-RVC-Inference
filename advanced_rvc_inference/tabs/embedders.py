...
class AdvancedEmbedderManager:
            ...
            if "onnx" not in model_name:
                ...
                self.model_info[onnx_name] = {
                    **self.model_info[model_name],
                    ...
                    "description": f"ONNX optimized {
                        self.model_info[model_name]['type']} model"}

    def _load_transformers_model(
            self, model_name: str) -> Tuple[nn.Module, Any, Any]:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to load fallback model: {fallback_e}")

    def _load_fairseq_model(
            self, model_name: str) -> Tuple[nn.Module, Any, Any]:
        ...
        print(
            f"Fairseq loading not implemented for {model_name}, falling back to transformers")
    ...
    def _load_custom_model(
            self, model_path: str) -> Tuple[nn.Module, Any, Any]:
    ...
    def get_model(self, model_name: str,
                  mode: str = "auto") -> Tuple[nn.Module, Any, Any, str]:
        ...
        if model_name not in self.models:
            ...
            if mode in self.loading_strategies:
                model, tokenizer, config = self.loading_strategies[mode](
                    model_name)
        ...
        try:
            ...
            with torch.no_grad():
                if mode == "whisper":
                    ...
                    embeddings = self._extract_whisper_embeddings(
                        audio_tensor, model, layer)
                elif "spin" in mode.lower():
                    # SPIN specific processing
                    embeddings = self._extract_spin_embeddings(
                        audio_tensor, model, layer)
                else:
                    # Standard transformer processing
                    embeddings = self._extract_standard_embeddings(
                        audio_tensor, model, layer)
    ...
    def _extract_standard_embeddings(
            self,
            audio: torch.Tensor,
            model: nn.Module,
    ...
    def _extract_whisper_embeddings(
            self,
            audio: torch.Tensor,
            model: nn.Module,
            layer: int) -> torch.Tensor:
...
def embedders_tab():
    with gr.Row():
        with gr.Column():
            ...
            gr.Markdown(
                i18n("Select and configure embedder models for voice conversion."))
            ...
            apply_embedder_btn = gr.Button(
                i18n("Load Model & Test Embedding"), variant="primary")
    ...
    def update_embedder_info(
        ...
        if embedder == "custom" and custom_path:
            ...
            error_msg = f"{
                i18n('Error')}: {
                i18n('Please specify a custom embedder path')}"
        ...
        try:
            ...
            if model_info:
                ...
                info_text += f"{
                    i18n('Model info not available for')}: {mapped_embedder}\n"
        ...
        if test_audio:
            try:
                ...
                status_msg = f"{
                    i18n('Successfully extracted embeddings')}: {
                    embeddings.shape}\n"
                status_msg += f"{
                    i18n('Processing time')}: {
                    extraction_time:.2f}s"
            ...
            status_msg = f"{
                i18n('Model loaded successfully')}: {mapped_embedder}\n"
    ...
    def extract_test_embeddings(test_audio, embedder, custom_path, layer):
        ...
        try:
            ...
            # Add sample values
            if embeddings.size > 0:
                sample_values = embeddings[0][:10] if len(
                    embeddings) > 0 else []
