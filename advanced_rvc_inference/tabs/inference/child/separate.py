...
def separate_tab():
    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            with gr.Accordion(translations["use_url"], open=False):
                with gr.Row():
                    url = gr.Textbox(label=translations["url_audio"], value="", placeholder="https://www.youtube.com/...", scale=4)
                    download_button = gr.Button(translations["downloads"], scale=1)
            with gr.Row():
    ...
    # Process Button
    with gr.Row():
        separate_button = gr.Button(translations["separator_tab"], variant="primary", size="lg")
    ...
    def update_ui_elements(*args):
        return separate_change(*args)
    
    change_inputs = [model_name, karaoke_model, reverb_model, enable_post_process, separate_backing, separate_reverb, enable_denoise]
    change_outputs = [
        karaoke_model,
        reverb_model,
        overlap, 
    ...
    model_name.change(fn=update_ui_elements, inputs=change_inputs, outputs=change_outputs)
    karaoke_model.change(fn=update_ui_elements, inputs=change_inputs, outputs=change_outputs)
    reverb_model.change(fn=update_ui_elements, inputs=change_inputs, outputs=change_outputs)
    separate_backing.change(fn=update_ui_elements, inputs=change_inputs, outputs=change_outputs)
    separate_reverb.change(fn=update_ui_elements, inputs=change_inputs, outputs=change_outputs)
    enable_denoise.change(fn=update_ui_elements, inputs=change_inputs, outputs=change_outputs)
    enable_post_process.change(fn=update_ui_elements, inputs=change_inputs, outputs=change_outputs)
