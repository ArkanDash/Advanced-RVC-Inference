...
class HubertModelONNX:
    ...
    def extract_features(self, source, padding_mask=None, output_layer=None):
        logits = self.model.run([self.model.get_outputs()[0].name, self.model.get_outputs()[
                                1].name], {"feats": source.detach().cpu().numpy()})
        return [torch.as_tensor(
            logits[int(output_layer != 9)], dtype=torch.float32, device=self.device)]
