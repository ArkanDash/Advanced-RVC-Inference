...
def load_model(filename):
    ...
    model = HubertModel(
        HubertConfig(
            **state['cfg']['model']),
        num_classes=int(
            state['model']['label_embs_concat'].shape[0]))
...
def softmax(x, dim, onnx_trace=False):
    return F.softmax(
        x.float(),
        dim=dim) if onnx_trace else F.softmax(
        x,
        dim=dim,
        dtype=torch.float32)
...
def log_softmax(x, dim, onnx_trace=False):
    return F.log_softmax(
        x.float(), dim=dim) if onnx_trace else F.log_softmax(
...
def with_incremental_state(cls):
    cls.__bases__ = (FairseqIncrementalState,) + \
        tuple(b for b in cls.__bases__ if b != FairseqIncrementalState)
    return cls
        ...
        if mod.training:
            if not is_conv:
                ...
                if mod.kernel_size == (1, 1):
                    ...
                    mask.bernoulli_(p)
                    mask = (
                        mask.unsqueeze(2).unsqueeze(3).repeat(
...
class FairseqDropout(nn.Module):
    ...
    def forward(self, x, inplace=False):
        return F.dropout(
            x, p=self.p, training=True, inplace=inplace) if self.p > 0 and (
            self.training or self.apply_during_inference) else x
...
class FairseqIncrementalState(object):
    ...
    def set_incremental_state(self, incremental_state, key, value):
        if incremental_state is not None:
            incremental_state[self._get_full_incremental_state_key(
                key)] = value
...
class FairseqDecoder(nn.Module):
    ...
    def get_normalized_probs_scriptable(
        ...
        return log_softmax(
            logits,
            dim=-1,
            onnx_trace=self.onnx_trace) if log_probs else softmax(
            logits,
            dim=-1,
    ...
    def set_beam_size(self, beam_size):
        if getattr(self, "_beam_size", -1) != beam_size:
            ...
            def apply_set_beam_size(module):
                if (module != self and hasattr(module,
                                               "set_beam_size") and module not in seen):
                    seen.add(module)
...
class MultiheadAttention(FairseqIncrementalDecoder):
    ...
    def _get_reserve_head_index(self, num_heads_to_keep: int):
        ...
        for i in range(self.num_heads):
            ...
            k_proj_heads_norm.append((self.k_proj.weight[start_idx:end_idx]).abs().sum(
            ).tolist() + (self.k_proj.bias[start_idx:end_idx]).abs().sum().tolist())
            q_proj_heads_norm.append((self.q_proj.weight[start_idx:end_idx]).abs().sum(
            ).tolist() + (self.q_proj.bias[start_idx:end_idx]).abs().sum().tolist())
            v_proj_heads_norm.append((self.v_proj.weight[start_idx:end_idx]).abs().sum(
            ).tolist() + (self.v_proj.bias[start_idx:end_idx]).abs().sum().tolist())
    ...
    def _add_bias(self, k, v, key_padding_mask, attn_mask, bsz):
        ...
        return torch.cat([k, self.bias_k.repeat(1, bsz, 1)]), torch.cat(
            [v, self.bias_v.repeat(1, bsz, 1)]), key_padding_mask, attn_mask

    def _append_zero_attn(self, k, v, key_padding_mask, attn_mask):
        zero_attn_shape = k.size()[:-2] + torch.Size([1]) + k.size()[-1:]
        key_padding_mask, attn_mask = self._pad_masks(
            key_padding_mask=key_padding_mask, attn_mask=attn_mask)
        return torch.cat([k, torch.zeros(zero_attn_shape, dtype=k.dtype, device=k.device)], dim=-2), torch.cat(
            [v, torch.zeros(zero_attn_shape, dtype=v.dtype, device=v.device)], dim=-2), key_padding_mask, attn_mask
    ...
    def forward(
        ...
        if self.self_attention:
            ...
            if key is None:
                ...
                if self.beam_size > 1 and bsz == key.size(1):
                    key = key.view(key.size(0), -
                                   1, self.beam_size, key.size(2))[:, :, 0, :]
                    if key_padding_mask is not None:
                        key_padding_mask = key_padding_mask.view(
                            -1, self.beam_size, key_padding_mask.size(1))[:, 0, :]
                k = self.k_proj(key)
                v = self.v_proj(key)
        else:
            assert key is not None and value is not None
        ...
        if k is not None:
            kv_bsz = k.size(1)
            k = (k.contiguous().view(-1, kv_bsz *
                 self.num_heads, self.head_dim).transpose(0, 1))

        if v is not None:
            v = (v.contiguous().view(-1, kv_bsz *
                 self.num_heads, self.head_dim).transpose(0, 1))
        ...
        if self.encoder_decoder_attention and bsz != kv_bsz:
            attn_weights = torch.einsum("bxhtd,bhsd->bxhts", q.view((kv_bsz, -
                                                                     1, self.num_heads) +
        ...
        if key_padding_mask is not None:
            attn_weights = attn_weights.view(
                bsz, self.num_heads, tgt_len, src_len)
            attn_weights = attn_weights.view(
                kv_bsz, -1, self.num_heads, tgt_len, src_len).masked_fill(
                key_padding_mask.unsqueeze(1).unsqueeze(2).unsqueeze(3).to(
                    torch.bool), float("-inf")) if not is_tpu else attn_weights.transpose(
                0, 2).masked_fill(
                    key_padding_mask, float("-inf")).transpose(
                        0, 2)
            attn_weights = attn_weights.view(
                bsz * self.num_heads, tgt_len, src_len)
        ...
        if self.encoder_decoder_attention and bsz != kv_bsz:
            attn = torch.einsum("bxhts,bhsd->bxhtd", attn_probs.view((kv_bsz, -
                                                                      1, self.num_heads) +
                                                                     attn_probs.size()[1:]), v.view((kv_bsz, self.num_heads) +
                                                                                                    v.size()[1:]))
            attn = attn.reshape((-1,) + attn.size()[-2:])
        else:
            attn = attn_probs.bmm(v)
        assert list(
            attn.size()) == [
            bsz *
            self.num_heads,
        ...
        attn = attn.contiguous().view(
            tgt_len,
            bsz,
            self.embed_dim) if self.onnx_trace and attn.size(1) == 1 else attn.transpose(
            0,
            1).contiguous().view(
            tgt_len,
            bsz,
    ...
    def _append_prev_key_padding_mask(
        ...
        if prev_key_padding_mask is not None and static_kv:
            ...
            if src_len > prev_key_padding_mask.size(1):
                filler = torch.zeros(
                    (batch_size,
                     src_len - prev_key_padding_mask.size(1)),
                    ...
                    if self.encoder_decoder_attention:
                        if input_buffer_k.size(
                            ...
                            input_buffer[k] = input_buffer_k.index_select(
                                0, new_order.reshape(-1, self.beam_size)[:, 0] // self.beam_size)
                        else:
                            input_buffer[k] = input_buffer_k.index_select(
                                0, new_order)
                    else:
...
def make_conv_pos(e, k, g):
    ...
    dropout = 0
    nn.init.normal_(pos_conv.weight, mean=0, std=math.sqrt(
        (4 * (1.0 - dropout)) / (k * e)))
    nn.init.constant_(pos_conv.bias, 0)
    return nn.Sequential(nn.utils.parametrizations.weight_norm(
        pos_conv, name="weight", dim=2), SamePad(k), nn.GELU())
...
def pad_to_multiple(x, multiple, dim=-1, value=0):
    ...
    if m.is_integer():
        return x, 0
    return F.pad(x, (*((0,) * (-1 - dim) * 2), 0, remainder),
...
def compute_mask_indices(
    ...
    for i in range(bsz):
        seed_i = int(
            hash(
                (seed, epoch, indices[i].item())) %
            1e6) if seed is not None and epoch is not None and indices is not None else None
        ...
        if num_mask_ver == 1:
            num_mask = max(min_masks, int(mask_prob * sz / float(mask_length) +
                           np.random.rand())) if padding_mask is not None else all_num_mask
        elif num_mask_ver == 2:
            num_mask = max(
                min_masks, int(
        ...
        if mask_type == "static":
            ...
            lengths = [max(1, int(round(x))) for x in rng.normal(
                mask_length, mask_other, size=num_mask)]
        elif mask_type == "poisson":
            lengths = [int(round(x))
                       for x in rng.poisson(mask_length, size=num_mask)]
        else:
            raise Exception
    ...
    if require_same_masks:
        target_len = max([len(m) for m in mask_idcs]) if add_masks else min(
            [len(m) for m in mask_idcs])
...
def prune_state_dict(state_dict, model_cfg):
    ...
    if model_cfg is not None:
        arch = (
            model_cfg._name if isinstance(
                model_cfg,
                DictConfig) else getattr(
                model_cfg,
                "arch",
                None))
    ...
    def create_pruning_pass(layers_to_keep, layer_name):
        keep_layers = sorted(int(layer_string)
                             for layer_string in layers_to_keep.split(","))
        mapping_dict = {}
        for i in range(len(keep_layers)):
            mapping_dict[str(keep_layers[i])] = str(i)

        return {"substitution_regex": re.compile(
            r"^{layer}.*\.layers\.(\d+)".format(layer=layer_name)), "mapping_dict": mapping_dict}
    ...
    for layer_name in state_dict.keys():
        ...
        for pruning_pass in pruning_passes:
            if original_layer_number in pruning_pass["mapping_dict"] and pruning_pass["substitution_regex"].search(
                    layer_name):
                substitution_match = pruning_pass["substitution_regex"].search(
                    layer_name)
                new_state_dict[(layer_name[: substitution_match.start(1)] +
                                pruning_pass["mapping_dict"][original_layer_number] +
                                layer_name[substitution_match.end(1):])] = state_dict[layer_name]
...
def get_activation_fn(activation):
    ...
    def gelu_accurate(x):
        if not hasattr(gelu_accurate, "_a"):
            ...
            return (0.5 * x * (1 + (gelu_accurate._a *
                    (x + 0.044715 * x.pow(3))).tanh()))
...
class SamePad(nn.Module):
    def __init__(self, kernel_size, causal=False):
        ...
        if causal:
            ...
            self.remove = int(kernel_size % 2 == 0)
...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.layer_norm_first:
            ...
            x, attn = self.self_attn(
                query=x, key=x, value=x, key_padding_mask=self_attn_padding_mask, need_weights=False)
...
class ConvolutionModule(nn.Module):
    def __init__(
        ...
        self.depthwise_conv = nn.Conv1d(
            channels, channels, depthwise_kernel_size, stride=1, padding=(
                depthwise_kernel_size - 1) // 2, groups=channels, bias=bias)
...
def apply_rotary_pos_emb(q, k, cos, sin, offset: int = 0):
    cos, sin = (cos[offset: q.shape[0] + offset, ...],
...
class ESPNETMultiHeadedAttention(nn.Module):
    ...
    def forward_qkv(self, query, key, value, **kwargs):
        ...
        return self.linear_q(query).view(n_batch, -
                                         1, self.h, self.d_k).transpose(1, 2), self.linear_k(key).view(n_batch, -
                                                                                                       1, self.h, self.d_k).transpose(1, 2), self.linear_v(value).view(n_batch, -
                                                                                                                                                                       1, self.h, self.d_k).transpose(1, 2)
    ...
    def forward(self, query, key, value, key_padding_mask=None, **kwargs):
        ...
        return self.forward_attention(
            v, (q @ k.transpose(-2, -1)) / math.sqrt(self.d_k), key_padding_mask).transpose(0, 1), None
...
class RelPositionMultiHeadedAttention(ESPNETMultiHeadedAttention):
    ...
    def rel_shift(self, x):
        x = torch.cat([torch.zeros((*x.size()[:3],
                                    1),
                                   device=x.device,
                                   dtype=x.dtype),
                       x],
                      dim=-1).view(*x.size()[:2],
                                   ...
                                   x.size(2))[:,
                                              ...
                                              1:].view_as(x)[:,
                                                             ...
                                                             : x.size(-1) // 2 + 1]
        if self.zero_triu:
            x = x * torch.ones((x.size(2),
                                x.size(3)),
                               device=x.device).tril(x.size(3) - x.size(2))[None,
                                                                            None,
                                                                            :,
                                                                            :]
        return x
    ...
    def forward(
        ...
        return self.forward_attention(v, (((q + self.pos_bias_u).transpose(1, 2) @ k.transpose(-2, -1)) + self.rel_shift(((q + self.pos_bias_v).transpose(1, 2) @ self.linear_pos(
            pos_emb).view(pos_emb.size(0), -1, self.h, self.d_k).transpose(1, 2).transpose(-2, -1)))) / math.sqrt(self.d_k), key_padding_mask).transpose(0, 1), None
...
class ConformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_padding_mask, position_emb=None):
        ...
        if self.pos_enc_type == "rel_pos":
            ...
            x, attn = self.self_attn(
                query=x, key=x, value=x, key_padding_mask=encoder_padding_mask, need_weights=False)
...
class TransformerSentenceEncoderWithAdapterLayer(
    ...
    def forward(
            ...
            att_args=None,
            corpus_key=None):
        x, (attn, layer_result) = super().forward(x=x, self_attn_mask=self_attn_mask,
                                                  self_attn_padding_mask=self_attn_padding_mask, need_weights=need_weights, att_args=att_args)
...
class TransformerEncoder(nn.Module):
    def build_encoder_layer(self, args, **kwargs):
        if args.layer_type == "transformer":
            ...
            if args.adp_trf_idx == "all":
                ...
                if kwargs.get("layer_idx", None) in list(
            ...
            layer = TransformerSentenceEncoderWithAdapterLayer(
                ...
                adapter_act_fn=args.adp_act_fn) if use_adp else TransformerSentenceEncoderLayer(
                embedding_dim=self.embedding_dim,
                ffn_embedding_dim=args.encoder_ffn_embed_dim,
                num_attention_heads=args.encoder_attention_heads,
                dropout=self.dropout,
    ...
    def __init__(self, args):
        ...
        self.layers = nn.ModuleList([self.build_encoder_layer(
            args, layer_idx=ii) for ii in range(args.encoder_layers)])
    ...
    def extract_features(
        ...
        x = F.dropout(
            x,
            p=self.dropout,
            training=self.training).transpose(
            0,
            1)
        layer_results = []
        r = None
...
class ConvFeatureExtractionModel(nn.Module):
    def __init__(
        ...
        def block(
            ...
            if is_layer_norm:
                return nn.Sequential(
                    make_conv(), nn.Dropout(
                        p=dropout), nn.Sequential(
                        TransposeLast(), Fp32LayerNorm(
                            dim, elementwise_affine=True), TransposeLast()), nn.GELU())
            elif is_group_norm:
                return nn.Sequential(
                    make_conv(), nn.Dropout(
                        p=dropout), Fp32GroupNorm(
                        dim, dim, affine=True), nn.GELU())
            else:
                return nn.Sequential(
                    make_conv(), nn.Dropout(
                        p=dropout), nn.GELU())
...
class HubertModel(BaseFairseqModel):
    def __init__(self, cfg, num_classes):
        ...
        self.post_extract_proj = (
            nn.Linear(
                self.embed,
                cfg.encoder_embed_dim) if self.embed != cfg.encoder_embed_dim else None)
    ...
    def apply_mask(self, x, padding_mask, target_list):
        ...
        if self.mask_prob > 0:
            mask_indices = torch.from_numpy(
                compute_mask_indices(
                    ...
                    min_space=self.mask_min_space)).to(
                x.device)
        ...
        if self.mask_channel_prob > 0:
            x[(torch.from_numpy(compute_mask_indices((B,
                                                      C),
                                                     None,
                                                     ...
                                                     min_space=self.mask_channel_min_space)).to(x.device).unsqueeze(1).expand(-1,
                                                                                                                              T,
                                                                                                                              -1))] = 0
    ...
    def compute_nce(self, x, pos, negs):
        ...
        logits = torch.cosine_similarity(x.float(), torch.cat(
            [pos.unsqueeze(0), negs], dim=0).float(), dim=-1).type_as(x)
    ...
    def forward_targets(self, features, target_list):
        ...
        return features, [t[:, (torch.arange(feat_tsz).float(
        ) * self.feat2tar_ratio).long()] for t in target_list]

    def forward_padding_mask(self, features, padding_mask):
        extra = padding_mask.size(1) % features.size(1)
        if extra > 0:
            padding_mask = padding_mask[:, :-extra]
        return padding_mask.view(padding_mask.size(
            0), features.size(1), -1).all(-1)
                 ...
                 t) in enumerate(
                    zip(
                        proj_x_m.chunk(
                            len(target_list),
                            dim=-
                            1) if self.untie_final_proj else [
                            proj_x_m for _ in range(
                                len(target_list))],
                        target_list))]
        else:
    ...
    def get_logits(self, net_output, is_masked=True):
        return [x.float() for x in (net_output["logit_m_list"]
                                    if is_masked else net_output["logit_u_list"]) if x is not None]
