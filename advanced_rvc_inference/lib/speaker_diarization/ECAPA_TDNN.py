...
class _BatchNorm1d(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.combine_batch_time:
            x = x.reshape(
                shape_or[0] * shape_or[1],
                shape_or[2]) if x.ndim == 3 else x.reshape(
                shape_or[0] * shape_or[1],
                shape_or[3],
                shape_or[2])
...
class AttentiveStatisticsPooling(nn.Module):
    def __init__(self, channels, attention_channels=128, global_context=True):
        ...
        self.tdnn = TDNNBlock(
            ...
            attention_channels,
            1,
            1) if global_context else TDNNBlock(
            channels,
            attention_channels,
            1,
            1)
    ...
    def forward(self, x, lengths=None):
        ...
        def _compute_statistics(x, m, dim=2, eps=self.eps):
            mean = (m * x).sum(dim)
            return mean, ((m * (x - mean.unsqueeze(dim)).pow(2)
                           ).sum(dim).clamp(eps)).sqrt()
        ...
        if self.global_context:
            mean, std = _compute_statistics(
                x, mask / mask.sum(dim=2, keepdim=True).float())
            attn = torch.cat([x, mean.unsqueeze(2).repeat(
                1, 1, L), std.unsqueeze(2).repeat(1, 1, L)], dim=1)
        else:
            attn = x
        ...
        mean, std = _compute_statistics(
            x, F.softmax(
                self.conv(
                    self.tanh(
                        self.tdnn(attn))).masked_fill(
                    mask == 0, float("-inf")), dim=2))
...
class ECAPA_TDNN(torch.nn.Module):
    def __init__(
        ...
        for i in range(1, len(channels) - 1):
            self.blocks.append(SERes2NetBlock(channels[i - 1],
                                              channels[i],
        ...
        self.mfa = TDNNBlock(channels[-2] * (len(channels) - 2),
                             channels[-1],
                             kernel_sizes[-1],
                             dilations[-1],
                             activation,
        ...
        self.asp = AttentiveStatisticsPooling(
            channels[-1], attention_channels=attention_channels, global_context=global_context)
        self.asp_bn = BatchNorm1d(input_size=channels[-1] * 2)
        self.fc = Conv1d(
            in_channels=channels[-1] * 2, out_channels=lin_neurons, kernel_size=1)
    ...
    def forward(self, x, lengths=None):
        ...
        return self.fc(self.asp_bn(
            self.asp(self.mfa(torch.cat(xl[1:], dim=1)), lengths=lengths))).transpose(1, 2)
...
class Classifier(torch.nn.Module):
    def __init__(
        ...
        for _ in range(lin_blocks):
            self.blocks.extend([_BatchNorm1d(input_size=input_size), Linear(
                input_size=input_size, n_neurons=lin_neurons)])
