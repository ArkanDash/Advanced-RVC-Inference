from ...lib.speaker_diarization.speechbrain import (MAIN_PROC_ONLY,
...
KEYS_MAPPING = {".mutihead_attn": ".multihead_attn",
    ...
    for replacement_old, replacement_new in mapping.items():
        for old_key in list(state_dict.keys()):
            if replacement_old in old_key:
                state_dict[old_key.replace(
                    replacement_old, replacement_new)] = state_dict.pop(old_key)
...
def register_checkpoint_hooks(cls, save_on_main_only=True):
    ...
    for name, method in cls.__dict__.items():
        if hasattr(method, "_speechbrain_saver"):
            DEFAULT_SAVE_HOOKS[cls] = main_process_only(
                method) if save_on_main_only else method
...
def fwd_default_precision(fwd=None, cast_inputs=torch.float32):
    ...
    def wrapper(*args, force_allow_autocast=False, **kwargs):
        return fwd(*
                   args, **kwargs) if force_allow_autocast else wrapped_fwd(*
                                                                            args, **kwargs)
...
class Filterbank(torch.nn.Module):
    def __init__(
        ...
        hz = self._to_hz(
            torch.linspace(
                self._to_mel(
                    self.f_min), self._to_mel(
                    self.f_max), self.n_mels + 2))
        ...
        self.all_freqs_mat = torch.linspace(
            0,
            self.sample_rate // 2,
            self.n_stft).repeat(
            self.f_central.shape[0],
            1)

    def forward(self, spectrogram):
        f_central_mat = self.f_central.repeat(
            self.all_freqs_mat.shape[1],
            1).transpose(
            0,
            1)
        band_mat = self.band.repeat(
            self.all_freqs_mat.shape[1],
            1).transpose(
            0,
            1)
        ...
        fbank_matrix = self._create_fbank_matrix(
            f_central_mat, band_mat).to(
            spectrogram.device)
        sp_shape = spectrogram.shape
        if len(sp_shape) == 4:
            spectrogram = spectrogram.permute(0, 3, 1, 2).reshape(
    ...
    def _triangular_filters(self, all_freqs, f_central, band):
        ...
        return torch.zeros(1, device=self.device_inp).max(
            (slope + 1.0).min(-slope + 1.0)).transpose(0, 1)
    ...
    def _gaussian_filters(
            ...
            f_central,
            band,
            smooth_factor=torch.tensor(2)):
        return (-0.5 * ((all_freqs - f_central) /
                (band / smooth_factor)) ** 2).exp().transpose(0, 1)
    ...
    def _amplitude_to_DB(self, x):
        ...
        return x_db.max((x_db.amax(dim=(-2, -1)) -
                        self.top_db).view(x_db.shape[0], 1, 1))
...
class ContextWindow(torch.nn.Module):
    def __init__(self, left_frames=0, right_frames=0):
        ...
        if self.right_frames > self.left_frames:
            self.kernel = torch.roll(
                self.kernel, self.right_frames - self.left_frames, 1)
    ...
    def forward(self, x):
        ...
        cw_x = torch.nn.functional.conv1d(
            x, self.kernel.to(
                x.device), groups=x.shape[1], padding=max(
...
class FilterProperties:
    ...
    def with_on_top(self, other, allow_approximate=True):
        ...
        if (self.causal or other.causal) and not (
                self.causal and other.causal):
            if allow_approximate:
        ...
        return FilterProperties(self_size +
                                (self.stride *
                                 (other_size -
                                  1)), self.stride *
                                other.stride, self.dilation *
                                other.dilation, self.causal)
...
class STFT(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        stft = stft.reshape(
            ...
            stft.shape[1],
            stft.shape[2],
            stft.shape[3]).permute(
            0,
            3,
            2,
            4,
            1) if len(or_shape) == 3 else stft.transpose(
            2,
            1)
...
class Deltas(torch.nn.Module):
    def __init__(self, input_size, window_length=5):
        ...
        self.register_buffer("kernel",
                             torch.arange(-self.n,
                                          self.n + 1,
                                          dtype=torch.float32).repeat(input_size,
                                                                      1,
                                                                      1),
                             )
...
class Fbank(torch.nn.Module):
    ...
    def forward(self, wav):
        ...
        if self.deltas:
            ...
            fbanks = torch.cat(
                [fbanks, delta1, self.compute_deltas(delta1)], dim=2)
...
class InputNormalization(torch.nn.Module):
    ...
    def forward(self, x, lengths, spk_ids=torch.tensor([]), epoch=0):
        ...
        for snt_id in range(N_batches):
            ...
            if self.norm_type == "sentence":
                out[snt_id] = (
                    x[snt_id] - current_mean.data) / current_std.data
                ...
                if self.training:
                    if spk_id not in self.spk_dict_mean:
                        ...
                        self.spk_dict_count[spk_id] = (
                            self.spk_dict_count[spk_id] + 1)
                        self.weight = (
                            1 / self.spk_dict_count[spk_id]) if self.avg_factor is None else self.avg_factor

                        self.spk_dict_mean[spk_id] = (
                            1 - self.weight) * self.spk_dict_mean[spk_id].to(current_mean) + self.weight * current_mean
                        self.spk_dict_std[spk_id] = (
                            1 - self.weight) * self.spk_dict_std[spk_id].to(current_std) + self.weight * current_std
        ...
        if self.norm_type == "batch" or self.norm_type == "global":
            current_mean = ddp_all_reduce(
                torch.stack(current_means).mean(
                    dim=0), torch.distributed.ReduceOp.AVG)
            current_std = ddp_all_reduce(
                torch.stack(current_stds).mean(
                    dim=0), torch.distributed.ReduceOp.AVG)
            ...
            if self.norm_type == "global":
                if self.training:
                    if self.count == 0:
                        ...
                        self.weight = (
                            1 / (self.count + 1)) if self.avg_factor is None else self.avg_factor
                        self.glob_mean = (
                            1 - self.weight) * self.glob_mean.to(current_mean) + self.weight * current_mean
                        self.glob_std = (
                            1 - self.weight) * self.glob_std.to(current_std) + self.weight * current_std
    ...
    def _compute_current_stats(self, x):
        current_std = x.std(
            dim=0).detach().data if self.std_norm else torch.tensor(
            [1.0], device=x.device)
        return x.mean(
            dim=0).detach().data if self.mean_norm else torch.tensor(
            [0.0], device=x.device), torch.max(
