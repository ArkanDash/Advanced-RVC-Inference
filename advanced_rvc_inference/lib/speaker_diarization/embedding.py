...
class SpeechBrainPretrainedSpeakerEmbedding:
    ...
    def dimension(self):
        *_, dimension = self.classifier_.encode_batch(
            torch.rand(1, 16000).to(self.device)).shape
        return dimension
    ...
    def min_num_samples(self):
        with torch.inference_mode():
            lower, upper = 2, round(
                0.5 * self.classifier_.audio_normalizer.sample_rate)
    ...
    def __call__(self, waveforms, masks=None):
        ...
        if masks is None:
            ...
            imasks = F.interpolate(
                ...
                dim=1) > 0.5
            signals = pad_sequence([waveform[imask].contiguous(
            ) for waveform, imask in zip(waveforms, imasks)], batch_first=True)
        ...
        embeddings = (
            self.classifier_.encode_batch(
                signals, wav_lens=wav_lens).squeeze(
                dim=1).cpu().numpy())
