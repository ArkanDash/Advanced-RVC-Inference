...
class Timeline:
    ...
    def __init__(self, segments=None, uri=None):
        ...
        self.segments_boundaries_ = SortedList(
            (boundary for segment in segments_set for boundary in segment))
        self.uri = uri
    ...
    def union(self, timeline):
        return Timeline(segments=self.segments_set_ |
                        timeline.segments_set_, uri=self.uri)
    ...
    def crop_iter(self, support, mode='intersection', returns_mapping=False):
        ...
        if isinstance(support, Segment):
            ...
            for yielded in self.crop_iter(
                    support, mode=mode, returns_mapping=returns_mapping):
                yield yielded
    ...
    def crop(self, support, mode='intersection', returns_mapping=False):
        if mode == 'intersection' and returns_mapping:
            ...
            for segment, mapped_to in self.crop_iter(
                    support, mode='intersection', returns_mapping=True):
    ...
    def overlapping_iter(self, t):
        for segment in self.segments_list_.irange(
                maximum=Segment(start=t, end=t)):
    ...
    def extrude(self, removed, mode='intersection'):
        ...
        return self.crop(removed.gaps(support=Timeline(
            [self.extent()], uri=self.uri)), mode=mode)
    ...
    def __repr__(self):
        return "<Timeline(uri=%s, segments=%s)>" % (
            self.uri, list(self.segments_list_))
    ...
    def copy(self, segment_func=None):
        ...
        return Timeline(segments=[segment_func(s)
                        for s in self.segments_list_], uri=self.uri)

    def extent(self):
        if self.segments_set_:
            segments_boundaries_ = self.segments_boundaries_
            return Segment(start=segments_boundaries_[
                           0], end=segments_boundaries_[-1])
...
class Segment:
    ...
    def __post_init__(self):
        if AUTO_ROUND_TIME:
            object.__setattr__(
                self, 'start', int(
                    self.start / SEGMENT_PRECISION + 0.5) * SEGMENT_PRECISION)
            object.__setattr__(
                self, 'end', int(
                    self.end / SEGMENT_PRECISION + 0.5) * SEGMENT_PRECISION)
    ...
    def __and__(self, other):
        return Segment(
            start=max(
                self.start, other.start), end=min(
                self.end, other.end))

    def intersects(self, other):
        return (
            self.start < other.start and other.start < self.end -
            SEGMENT_PRECISION) or (
    ...
    def __or__(self, other):
        ...
        return Segment(
            start=min(
                self.start, other.start), end=max(
                self.end, other.end))
    ...
    def _str_helper(self, seconds):
        ...
        return '%s%02d:%02d:%02d.%03d' % (
            '-' if negative else ' ', hours, minutes, seconds, td.microseconds / 1000)
...
class SlidingWindow:
    ...
    def samples(self, from_duration, mode='strict'):
        if mode == 'strict':
            return int(
                np.floor(
                    (from_duration - self.duration) / self.step)) + 1
        elif mode == 'loose':
        ...
        if isinstance(focus, Timeline):
            ...
            if return_ranges:
                ...
                for i, s in enumerate(focus.support()):
                    rng = self.crop(
                        s, mode=mode, fixed=fixed, return_ranges=True)
            ...
            return np.unique(np.hstack([self.crop(
                s, mode=mode, fixed=fixed, return_ranges=False) for s in focus.support()]))
    ...
    def segment_to_range(self, segment):
        return self.closest_frame(
            segment.start), int(
            segment.duration / self.step) + 1
    ...
    def __call__(self, support, align_last=False):
        ...
        for segment in segments:
            ...
            if align_last and last.end < segment.end:
                yield Segment(start=segment.end - self.duration, end=segment.end)
