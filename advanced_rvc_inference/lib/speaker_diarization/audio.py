...
class Audio:
    ...
    def power_normalize(waveform):
        return waveform / (waveform.square().mean(dim=-
                           1, keepdim=True).sqrt() + 1e-8)
    ...
    def validate_file(file):
        ...
        if "waveform" in file:
            ...
            if len(
                    waveform.shape) != 2 or waveform.shape[0] > waveform.shape[1]:
                raise ValueError
    ...
    def get_duration(self, file):
        ...
        if "waveform" in file:
            ...
            info = file["torchaudio.info"] if "torchaudio.info" in file else get_torchaudio_info(
    ...
    def crop(self, file, segment, duration=None, mode="raise"):
        ...
        if mode == "raise":
            ...
            if end_frame > frames + math.ceil(0.001 * sample_rate):
                raise ValueError
            else:
        ...
        if "waveform" in file:
            ...
            try:
                data, _ = torchaudio.load(
                    file["audio"], frame_offset=start_frame, num_frames=num_frames, backend=self.backend)
                if isinstance(file["audio"], IOBase):
                    file["audio"].seek(0)
            except RuntimeError:
                if isinstance(file["audio"], IOBase):
        ...
        if mode == "pad":
            data = torch.nn.functional.pad(data, (pad_start, pad_end))
