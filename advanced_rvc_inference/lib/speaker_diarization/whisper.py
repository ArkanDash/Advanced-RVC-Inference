...
from assets.config.variables import configs, logger
...
LANGUAGES = {
    ...
    "su": "sundanese",
    "yue": "cantonese"}
TO_LANGUAGE_CODE = {**{language: code for code,
                       language in LANGUAGES.items()},
                    "burmese": "my",
                    "valencian": "ca",
                    "flemish": "nl",
                    "haitian": "ht",
...
def find_alignment(
    ...
    tokens = torch.tensor([*
                           tokenizer.sot_sequence, tokenizer.no_timestamps, *
                           text_tokens, tokenizer.eot]).to(model.device)

    QKs = [None] * model.dims.n_text_layer
    hooks = [block.cross_attn.register_forward_hook(lambda _, ins, outs, index=i: QKs.__setitem__(
        index, outs[-1][0])) for i, block in enumerate(model.decoder.blocks)]

    with torch.no_grad():
        token_probs = model(mel.unsqueeze(0), tokens.unsqueeze(0))[0][len(
            tokenizer.sot_sequence):, : tokenizer.eot].softmax(dim=-1)
        text_token_probs = token_probs[np.arange(
            len(text_tokens)), text_tokens].tolist()
    ...
    if not (opencl.is_available() or directml.is_available()):
        ...
        alignment_indices = [(l, h) for l in range(model.alignment_heads.size(
            0)) for h in range(model.alignment_heads.size(1)) if model.alignment_heads[l, h]]

    weights = (torch.stack([QKs[_l][_h] for _l, _h in alignment_indices])[
               :, :, : num_frames // 2] * qk_scale).softmax(dim=-1)
    ...
    text_indices, time_indices = dtw(-weights.mean(axis=0)
                                     [len(tokenizer.sot_sequence): -1])
    ...
    jump_times = time_indices[np.pad(np.diff(
        text_indices), (1, 0), constant_values=1).astype(bool)] / TOKENS_PER_SECOND
    ...
    return [WordTiming(word,
            ...
            probability in zip(words,
                               word_tokens,
                               jump_times[word_boundaries[:-1]],
                               jump_times[word_boundaries[1:]],
                               [np.mean(text_token_probs[i:j]) for i,
                                   j in zip(word_boundaries[:-1],
                                            word_boundaries[1:])])]
        ...
        if len(words) > 0:
            if words[0]["end"] - last_speech_timestamp > median_duration * 4 and (
                ...
                if (len(words) > 1 and words[1]["end"] -
                        words[1]["start"] > max_duration):
                    words[0]["end"] = words[1]["start"] = max(
            ...
            if (segment["end"] > words[-1]["start"]
                    and segment["end"] + 0.5 < words[-1]["end"]):
                words[-1]["end"] = max(words[-1]["start"] +
                                       median_duration, segment["end"])
            else:
...
def log_mel_spectrogram(audio, n_mels=80, padding=0, device=None):
    ...
    if not torch.is_tensor(audio):
        if isinstance(audio, str):
            ...
            audio = load_audio(
                audio, sample_rate=SAMPLE_RATE).astype(
                np.float32)
    ...
    log_spec = (mel_filters(audio.device, n_mels) @
                fft[..., :-1].abs() ** 2).clamp(min=1e-10).log10()
    ...
    if torch.is_tensor(array):
        if array.shape[axis] > length:
            array = array.index_select(
                dim=axis, index=torch.arange(
                    length, device=array.device))
...
def get_end(segments):
    return next((w["end"] for s in reversed(segments) for w in reversed(
        s["words"])), segments[-1]["end"] if segments else None)
...
def transcribe_function(
    ...
    if decode_options.get("language", None) is None:
        if not model.is_multilingual:
            ...
            if verbose is not None:
                logger.info(f"{LANGUAGES[decode_options['language']].title()}")
    ...
    def decode_with_fallback(segment):
        ...
        for t in temperatures:
            ...
            decode_result = model.decode(
                segment, DecodingOptions(
                    **kwargs, temperature=t))
            needs_fallback = False

            if (compression_ratio_threshold is not None and decode_result.compression_ratio >
                    compression_ratio_threshold):
                needs_fallback = True
            if (logprob_threshold is not None and decode_result.avg_logprob <
                    logprob_threshold):
                needs_fallback = True
            if (no_speech_threshold is not None and decode_result.no_speech_prob >
                    no_speech_threshold and logprob_threshold is not None and decode_result.avg_logprob < logprob_threshold):
                needs_fallback = False
    ...
    with tqdm.tqdm(total=content_frames, unit="frames", disable=verbose is not False) as pbar:
        ...
        while clip_idx < len(seek_clips):
            ...
            mel_segment = pad_or_trim(
                mel_segment, N_FRAMES).to(
                model.device).to(dtype)

            if carry_initial_prompt:
                decode_options["prompt"] = initial_prompt_tokens + all_tokens[max(
                    len(initial_prompt_tokens), prompt_reset_since):][-remaining_prompt_length:]
            else:
            ...
            if no_speech_threshold is not None:
                ...
                if (logprob_threshold is not None and result.avg_logprob >
                        logprob_threshold):
                    should_skip = False
            ...
            if len(consecutive) > 0:
                ...
                for current_slice in slices:
                    ...
                    current_segments.append(new_segment(start=time_offset + (sliced_tokens[0].item() - tokenizer.timestamp_begin) * time_precision, end=time_offset + (
                        sliced_tokens[-1].item() - tokenizer.timestamp_begin) * time_precision, tokens=sliced_tokens, result=result))
                    last_slice = current_slice

                if single_timestamp_ending:
                    seek += segment_size
                else:
                    seek += (tokens[last_slice - 1].item() -
                             tokenizer.timestamp_begin) * input_stride
            else:
                duration = segment_duration

                timestamps = tokens[timestamp_tokens.nonzero().flatten()]
                if (len(timestamps) >
                        0 and timestamps[-1].item() != tokenizer.timestamp_begin):
                    duration = (
                        timestamps[-1].item() - tokenizer.timestamp_begin) * time_precision
                ...
                if hallucination_silence_threshold is not None:
                    ...
                    if not single_timestamp_ending:
                        ...
                        if last_word_end is not None and last_word_end > time_offset:
                            seek = round(
                                last_word_end *
                                FRAMES_PER_SECOND) if (
                                window_end_time -
                                last_word_end) > threshold else (
                                previous_seek +
                                segment_size)
                    ...
                    for si in range(len(current_segments)):
                        ...
                        if is_segment_anomaly(segment):
                            ...
                            hal_next_start = next_segment["words"][0]["start"] if next_segment is not None else (
                                time_offset + segment_duration)

                            if (
                                    segment["start"] -
                                    hal_last_end > threshold or segment["start"] < threshold or segment["start"] -
                                    time_offset < 2.0) and (
                                    hal_next_start -
                                    segment["end"] > threshold or is_segment_anomaly(next_segment) or window_end_time -
                                    segment["end"] < 2.0):
                                seek = round(
                                    max(time_offset + 1, segment["start"]) * FRAMES_PER_SECOND)
                                if content_duration - \
                                        segment["end"] < threshold:
                                    seek = content_frames
            ...
            all_segments.extend([{"id": i,
                                  **segment} for i,
                                 segment in enumerate(current_segments,
    ...
    return dict(text=tokenizer.decode(all_tokens[len(
        initial_prompt_tokens):]), segments=all_segments, language=language)
...
def sinusoids(length, channels, max_timescale=10000):
    ...
    scaled_time = torch.arange(length)[:, np.newaxis] * (-(np.log(max_timescale) / (
        channels // 2 - 1)) * torch.arange(channels // 2)).exp()[np.newaxis, :]
...
def detect_language_function(model, mel, tokenizer=None):
    ...
    n_audio = mel.shape[0]
    logits = model.logits(torch.tensor(
        [[tokenizer.sot]] * n_audio).to(mel.device), mel)[:, 0]
    ...
    language_probs = [
        {
            ...
            c in zip(
                ...
                tokenizer.all_language_codes)} for i in range(n_audio)]
...
def get_encoding(name="gpt2", num_languages=99):
    ...
    ranks = {base64.b64decode(token): int(rank) for token, rank in (
        line.split() for line in open(vocab_path) if line)}
...
class Conv1d(nn.Conv1d):
    def _conv_forward(self, x, weight, bias):
        return super()._conv_forward(
            x, weight.to(
                x.dtype), None if bias is None else bias.to(
                x.dtype))
...
class TextDecoder(nn.Module):
    def __init__(self, n_vocab, n_ctx, n_state, n_head, n_layer):
        ...
        self.blocks = nn.ModuleList([ResidualAttentionBlock(
            n_state, n_head, cross_attention=True) for _ in range(n_layer)])
        self.ln = LayerNorm(n_state)
        self.register_buffer("mask", torch.empty(
            n_ctx, n_ctx).fill_(-np.inf).triu_(1), persistent=False)

    def forward(self, x, xa, kv_cache=None):
        offset = next(iter(kv_cache.values())).shape[1] if kv_cache else 0
        x = (self.token_embedding(
            x) + self.positional_embedding[offset: offset + x.shape[-1]]).to(xa.dtype)

        for block in self.blocks:
            x = block(x, xa, mask=self.mask, kv_cache=kv_cache)

        x = self.ln(x)
        return x @ self.token_embedding.weight.to(
            x.dtype).transpose(0, 1).float()
...
class Whisper(nn.Module):
    def __init__(self, dims):
        ...
        self.register_buffer("alignment_heads", all_heads if opencl.is_available(
        ) or directml.is_available() else all_heads.to_sparse(), persistent=False)

    def set_alignment_heads(self, dump):
        alignment = torch.from_numpy(
            np.frombuffer(
                gzip.decompress(
                    base64.b85decode(dump)),
                dtype=bool).copy()).reshape(
            self.dims.n_text_layer,
            self.dims.n_text_head)
        if not (opencl.is_available() or directml.is_available()):
            alignment = alignment.to_sparse()
    ...
    def install_kv_cache_hooks(self, cache=None):
        ...
        def save_to_cache(module, _, output):
            cache[module] = output if module not in cache or output.shape[1] > self.dims.n_text_ctx else torch.cat(
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, n_state, n_head, cross_attention=False):
        ...
        self.cross_attn = (
            MultiHeadAttention(
                n_state,
                n_head) if cross_attention else None)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, xa=None, mask=None, kv_cache=None):
        k, v = (self.key(x if xa is None else xa), self.value(x if xa is None else xa)
                ) if kv_cache is None or xa is None or self.key not in kv_cache else (kv_cache[self.key], kv_cache[self.value])
    ...
    def qkv_attention(self, q, k, v, mask=None):
        ...
        q, k, v = q.view(*
                         q.shape[:2], self.n_head, -
                         1).permute(0, 2, 1, 3), k.view(*
                                                        k.shape[:2], self.n_head, -
                                                        1).permute(0, 2, 1, 3), v.view(*
                                                                                       v.shape[:2], self.n_head, -
                                                                                       1).permute(0, 2, 1, 3)
        ...
        return (F.softmax(qk, dim=-1).to(q.dtype) @ v).permute(0,
                                                               2, 1, 3).flatten(start_dim=2), qk.detach()
...
class PyTorchInference(Inference):
    def __init__(self, model, initial_token_length):
        ...
        self.kv_modules = [
            block.attn.key for block in self.model.decoder.blocks] + [
            block.attn.value for block in self.model.decoder.blocks]
        ...
        def scores(logprobs, lengths):
            ...
            for logprob, length in zip(logprobs, lengths):
                result.append(logprob /
                              (length if self.length_penalty is None else ((5 +
                                                                            length) /
                                                                           6) ** self.length_penalty))
            return result

        return [np.argmax(scores(p, l)) for p, l in zip(
            sum_logprobs, [[len(t) for t in s] for s in tokens])]
...
class GreedyDecoder(TokenDecoder):
    ...
    def update(self, tokens, logits, sum_logprobs):
        next_tokens = logits.argmax(
            dim=-
            1) if self.temperature == 0 else (
            Categorical(
                logits=(
                    logits /
                    self.temperature).cpu() if opencl.is_available() else (
                    logits /
                    self.temperature))).sample().to(
                        logits.device)

        logprobs = F.log_softmax(logits.float(), dim=-1)
        sum_logprobs += logprobs[torch.arange(logprobs.shape[0]),
                                 next_tokens] * (tokens[:, -1] != self.eot)
...
class BeamSearchDecoder(TokenDecoder):
    ...
    def update(self, tokens, logits, sum_logprobs):
        ...
        return torch.tensor(next_tokens, device=tokens.device), all(
    ...
    def finalize(self, preceding_tokens, sum_logprobs):
        ...
        return [[torch.tensor(seq) for seq in sequences.keys()] for sequences in self.finished_sequences], [
...
class ApplyTimestampRules(LogitFilter):
    ...
    def apply(self, logits, tokens):
        ...
        for k in range(tokens.shape[0]):
            ...
            timestamps = sampled_tokens[sampled_tokens.ge(
                self.tokenizer.timestamp_begin)]

            if timestamps.numel() > 0:
                logits[k, self.tokenizer.timestamp_begin: timestamps[-1]
                       if last_was_timestamp and not penultimate_was_timestamp else (timestamps[-1] + 1)] = -np.inf
        ...
        logprobs = F.log_softmax(logits.float(), dim=-1)
        for k in range(tokens.shape[0]):
            if logprobs[k, self.tokenizer.timestamp_begin:].logsumexp(
                    dim=-1) > logprobs[k, : self.tokenizer.timestamp_begin].max():
                logits[k, : self.tokenizer.timestamp_begin] = -np.inf
...
class DecodingTask:
        ...
        if prefix := self.options.prefix:
            prefix_tokens = (
                self.tokenizer.encode(
                    " " +
                    prefix.strip()) if isinstance(
                    prefix,
                    str) else prefix)
            if self.sample_len is not None:
                prefix_tokens = prefix_tokens[-(self.n_ctx //
                                                2 - self.sample_len):]
            tokens = tokens + prefix_tokens

        if prompt := self.options.prompt:
            tokens = ([self.tokenizer.sot_prev] + (self.tokenizer.encode(" " + prompt.strip())
                      if isinstance(prompt, str) else prompt)[-(self.n_ctx // 2 - 1):] + tokens)
    ...
    def _get_suppress_tokens(self):
        ...
        suppress_tokens.extend([self.tokenizer.transcribe,
    ...
    def _get_audio_features(self, mel):
        ...
        audio_features = mel if mel.shape[-2:] == (
            self.model.dims.n_audio_ctx, self.model.dims.n_audio_state) else self.model.encoder(mel)
        ...
        try:
            for i in range(self.sample_len):
                ...
                if (i == 0 and self.tokenizer.no_speech is not None):
                    probs_at_sot = logits[:,
                                          self.sot_index].float().softmax(dim=-1)
                    no_speech_probs = probs_at_sot[:,
                                                   self.tokenizer.no_speech].tolist()

                logits = logits[:, -1]
                for logit_filter in self.logit_filters:
                    logit_filter.apply(
                        logits.to("cpu") if opencl.is_available() else logits, tokens)
    ...
    def run(self, mel):
        ...
        if self.options.task == "lang_id":
            return [
                DecodingResult(
                    ...
                    language=language,
                    language_probs=probs) for features,
                language,
                probs in zip(
                    audio_features,
                    languages,
        ...
        tokens = [[t[self.sample_begin: (t == tokenizer.eot).nonzero()[
            0, 0]] for t in s] for s in tokens]

        selected = self.sequence_ranker.rank(tokens, sum_logprobs)
        tokens = [t[i].tolist() for i, t in zip(selected, tokens)]

        fields = ([tokenizer.decode(t).strip() for t in tokens],
                  languages,
                  tokens,
                  audio_features,
                  [lp / (len(t) + 1) for t,
        ...
        return [
            DecodingResult(
                ...
                compression_ratio=compression_ratio(text)) for text,
            language,
            tokens,
            features,
            avg_logprob,
            no_speech_prob in zip(
                *fields)]
...
class Tokenizer:
    def __init__(
        ...
        if self.task is not None:
            sot_sequence.append(
                self.special_tokens["<|transcribe|>"] if self.task == "transcribe" else self.special_tokens["<|translate|>"])
    ...
    def decode(self, token_ids, **kwargs):
        return self.encoding.decode(
            [t for t in token_ids if t < self.timestamp_begin], **kwargs)
    ...
    def split_tokens_on_unicode(self, tokens):
        ...
        for token in tokens:
            ...
            if (replacement_char not in decoded or self.decode_with_timestamps(tokens)[
                    unicode_offset + decoded.index(replacement_char)] == replacement_char):
    ...
    def split_tokens_on_spaces(self, tokens):
        ...
        for subword, subword_tokens in zip(subwords, subword_tokens_list):
            if (subword_tokens[0] >= self.eot) or (subword.startswith(" ")) or (
                    subword.strip() in string.punctuation) or len(words) == 0:
