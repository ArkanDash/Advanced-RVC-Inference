...
warnings.filterwarnings("ignore", category=UserWarning)audio
...
class Resample(torch.nn.Module):
    ...
    def forward(self, waveforms):
        ...
        return resampled_waveform.squeeze(
            1) if unsqueezed else resampled_waveform.transpose(1, 2)
...
class AudioNormalizer:
    ...
    def __call__(self, audio, sample_rate):
        ...
        return self._mix(
            self._cached_resamplers[sample_rate](
...
class Pretrained(torch.nn.Module):
    ...
    def __init__(
        ...
        for arg, default in {"device": "cpu", "data_parallel_count": -1, "data_parallel_backend": False, "distributed_launch": False, "distributed_backend": "nccl", "jit": False, "jit_module_keys": None,
                             "compile": False, "compile_module_keys": None, "compile_mode": "reduce-overhead", "compile_using_fullgraph": False, "compile_using_dynamic_shape_tracing": False}.items():
    ...
    def _compile(self):
        ...
        if self.compile:
            compile_module_keys = set(
                self.mods) if self.compile_module_keys is None else set(
                self.compile_module_keys)

        jit_module_keys = set()
        if self.jit:
            jit_module_keys = set(
                self.mods) if self.jit_module_keys is None else set(
                self.jit_module_keys)
    ...
    def _wrap_distributed(self):
        if not self.distributed_launch and not self.data_parallel_backend:
            ...
            for name, module in self.mods.items():
                if any(p.requires_grad for p in module.parameters()):
                    self.mods[name] = DP(module) if self.data_parallel_count == -1 else DP(
    ...
    def from_hparams(
        ...
        if pretrainer is not None:
            run_on_main(
                pretrainer.collect_files, kwargs={
...
class EncoderClassifier(Pretrained):
    ...
    def encode_batch(self, wavs, wav_lens=None, normalize=False):
        ...
        embeddings = self.mods.embedding_model(self.mods.mean_var_norm(
            self.mods.compute_features(wavs), wav_lens), wav_lens)
    ...
    def classify_batch(self, wavs, wav_lens=None):
        out_prob = self.mods.classifier(
            self.encode_batch(
                wavs, wav_lens)).squeeze(1)
        score, index = out_prob.max(dim=-1)

        return out_prob, score, index, self.hparams.label_encoder.decode_torch(
            index)
