...
def istft(frames, framesize, hopsize):
    ...
    A = asymmetric_analysis_window(
        analysis_window_size,
        synthesis_window_size) if analysis_window_size != synthesis_window_size else symmetric_window(analysis_window_size)
    S = asymmetric_synthesis_window(
        analysis_window_size,
        synthesis_window_size) if analysis_window_size != synthesis_window_size else symmetric_window(synthesis_window_size)
...
def asymmetric_synthesis_window(analysis_window_size, synthesis_window_size):
    ...
    window[n - m - m:n - m] = np.square(right[:m]) / \
        symmetric_window(2 * n - 2 * m)[n - m - m:n - m]
    window[-m:] = right[-m:]
...
def normalize(frames, frames0):
    for i in range(len(frames)):
        ...
        if b == 0:
            continue
        frames[i] = np.real(frames[i]) * np.sqrt(a / b) + \
            1j * np.imag(frames[i])
...
def shiftpitch(frames, factors, samplerate):
    for i in range(len(frames)):
        magnitudes = np.vstack(
            [resample(np.real(frames[i]), factor) for factor in factors])
        frequencies = np.vstack(
            [resample(np.imag(frames[i]), factor) * factor for factor in factors])
...
def encode(frames, framesize, hopsize, samplerate):
    ...
    for m, frame in enumerate(frames):
        ...
        i = np.arange(N)
        data[m] = np.abs(frame) + 1j * \
            ((i + (wrap(delta - i * phaseinc) / phaseinc)) * freqinc)
...
def decode(frames, framesize, hopsize, samplerate):
    ...
    freqinc = samplerate / analysis_framesize
    phaseinc = 2 * np.pi * hopsize / analysis_framesize
    timeshift = 2 * np.pi * synthesis_framesize * \
        np.arange(N) / N if synthesis_framesize != analysis_framesize else 0
...
class StftPitchShift:
    ...
    def shiftpitch(
        ...
        if np.issubdtype(dtype, np.integer):
            ...
            raise TypeError('not np.issubdtype(dtype, np.floating)')
        ...
        if np.issubdtype(dtype, np.integer):
            a, b = np.iinfo(dtype).min, np.iinfo(dtype).max
            output = (((output + 1) / 2) * (b - a) +
                      a).clip(a, b).astype(dtype)
