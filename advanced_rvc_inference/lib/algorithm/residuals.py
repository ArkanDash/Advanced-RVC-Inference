...
class ResBlockBase(torch.nn.Module):
    ...
    def forward(self, x, x_mask=None):
        for c1, c2 in zip(self.convs1, self.convs2):
            x = c2(apply_mask(torch.nn.functional.leaky_relu(c1(apply_mask(
                torch.nn.functional.leaky_relu(x, LRELU_SLOPE), x_mask)), LRELU_SLOPE), x_mask)) + x
    ...
    def remove_weight_norm(self):
        for conv in self.convs1 + self.convs2:
            if hasattr(
                    conv,
                    "parametrizations") and "weight" in conv.parametrizations:
...
class ElementwiseAffine(torch.nn.Module):
    ...
    def forward(self, x, x_mask, reverse=False, **kwargs):
        if not reverse:
            return ((self.m + self.logs.exp() * x) *
                    x_mask), (self.logs * x_mask).sum(dim=[1, 2])
        else:
            return (x - self.m) * (-self.logs).exp() * x_mask
...
class ResidualCouplingLayer(torch.nn.Module):
    ...
    def forward(self, x, x_mask, g=None, reverse=False):
        ...
        if not reverse:
            return torch.cat(
                [x0, (m + x1 * logs.exp() * x_mask)], 1), logs.sum(dim=[1, 2])
        else:
