...
from ...lib.algorithm.encoders import PosteriorEncoder, TextEncoder
from ...lib.algorithm.commons import rand_slice_segments, slice_segments
import os
import sys
import warnings
...
class Synthesizer(torch.nn.Module):
            ...
            return (
                self.dec(
                    z_slice, slice_segments(
                        pitchf, ids_slice, self.segment_size, 2), g=g) if self.use_f0 else self.dec(
                    z_slice, g=g)), ids_slice, x_mask, y_mask, (z, self.flow(
                        z, y_mask, g=g), m_p, logs_p, m_q, logs_q)
        else:
            return None, None, x_mask, None, (None,
                                              None, m_p, logs_p, None, None)
    ...
    def infer(
        ...
        o = self.dec(
            z * x_mask,
            nsff0,
            g=g) if self.use_f0 else self.dec(
            z * x_mask,
            g=g)
