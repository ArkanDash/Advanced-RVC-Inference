...
class WaveNet(torch.nn.Module):
    ...
    def forward(self, x, x_mask, g=None):
        ...
        for i in range(self.n_layers):
            ...
            g_l = (g[:, i * 2 * self.hidden_channels: (i + 1) * 2 *
                   self.hidden_channels, :] if g is not None else 0)
            res_skip_acts = self.res_skip_layers[i](
                self.drop(
    ...
    def remove_weight_norm(self):
        if self.gin_channels != 0:
            if hasattr(
                    self.cond_layer,
                    "parametrizations") and "weight" in self.cond_layer.parametrizations:
                parametrize.remove_parametrizations(
                    self.cond_layer, "weight", leave_parametrized=True)
            else:
        ...
        for l in self.in_layers:
            if hasattr(
                    l,
                    "parametrizations") and "weight" in l.parametrizations:
                parametrize.remove_parametrizations(
                    l, "weight", leave_parametrized=True)
            else:
