...
from ...lib.algorithm.attentions import FFN, MultiHeadAttention
import math
import os
import sys
import warnings
    ...
    def forward(self, x, x_mask):
        ...
        for i in range(self.n_layers):
            ...
            x = self.norm_layers_2[i](
                x + self.drop(self.ffn_layers[i](x, x_mask)))
...
class TextEncoder(torch.nn.Module):
    def __init__(
        ...
        self.emb_pitch = torch.nn.Embedding(
            256, hidden_channels) if f0 else None
        self.emb_energy = torch.nn.Linear(
            1, hidden_channels) if energy else None
    ...
    def forward(self, phone, pitch, lengths, energy):
        ...
        m, logs = (self.proj(self.encoder(x * x_mask, x_mask))
                   * x_mask).split(self.out_channels, dim=1)
...
class PosteriorEncoder(torch.nn.Module):
    ...
    def forward(self, x, x_lengths, g=None):
        ...
        m, logs = (self.proj(self.enc((self.pre(x) * x_mask), x_mask, g=g))
                   * x_mask).split(self.out_channels, dim=1)

        return ((m + torch.randn_like(m) * logs.exp())
                * x_mask), m, logs, x_mask
