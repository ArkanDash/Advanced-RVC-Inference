...
def rand_slice_segments(x, x_lengths=None, segment_size=4):
    ...
    ids_str = (torch.rand([b]).to(device=x.device) *
               (x_lengths - segment_size + 1)).to(dtype=torch.long)
...
def fused_add_tanh_sigmoid_multiply(input_a, input_b, n_channels):
    ...
    return in_act[:, :n_channels_int, :].tanh(
    ) * in_act[:, n_channels_int:, :].sigmoid()
...
def sequence_mask(length, max_length=None):
    ...
    return torch.arange(
        max_length,
        dtype=length.dtype,
        device=length.device).unsqueeze(0) < length.unsqueeze(1)
