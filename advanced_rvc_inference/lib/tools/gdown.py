    ...
    for pattern in (
        ...
        if match:
            ...
            if pattern == r'href="/open\?id=([^"]+)"':
                url = (
                    codecs.decode(
                        "uggcf://qevir.hfrepbagrag.tbbtyr.pbz/qbjaybnq?vq=",
                        "rot13") +
                    url +
                    "&confirm=t&uuid=" +
                    re.search(
                ...
                url = codecs.decode("uggcf://qbpf.tbbtyr.pbz",
                                    "rot13") + url.replace("&", "&")
            return url
...
def gdown_download(url=None, output=None):
    ...
    if url is None:
        raise ValueError(translations["gdown_value_error"])
    ...
    if file_id:
        url = f"{
            codecs.decode(
                'uggcf://qevir.tbbtyr.pbz/hp?vq=',
                'rot13')}{file_id}"
        url_origin = url

        sess, cookies_file = _get_session(
            use_cookies=True, return_cookies_file=True)
        ...
        while 1:
            ...
            if url == url_origin and res.status_code == 500:
                url = f"{
                    codecs.decode(
                        'uggcf://qevir.tbbtyr.pbz/bcra?vq=',
                        'rot13')}{gdrive_file_id}"
                continue

            os.makedirs(os.path.dirname(cookies_file), exist_ok=True)
            with open(cookies_file, "w") as f:
                json.dump([(k, v) for k, v in sess.cookies.items()
                          if not k.startswith("download_warning_")], f, indent=2)
        ...
        if gdrive_file_id and is_gdrive_download_link:
            ...
            filename_from_url = (
                re.search(
                    r"filename\*=UTF-8''(.*)",
                    content_disposition) or re.search(
                    r'filename=["\']?(.*?)["\']?$',
                    content_disposition)).group(1).replace(
                os.path.sep,
                "_")
        else:
