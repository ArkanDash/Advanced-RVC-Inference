...
def decrypt_attr(attr, key):
    attr = codecs.latin_1_decode(aes_cbc_decrypt(
        attr, a32_to_str(key)))[0].rstrip('\0')
...
def mega_download_file(file_handle, file_key, dest_path=None):
    ...
    if 'g' not in file_data:
        raise Exception(translations["file_not_access"])
    ...
    if (file_mac[0] ^ file_mac[1], file_mac[2] ^ file_mac[3]) != file_key[6:8]:
        raise ValueError(translations["mac_not_match"])
...
def mega_download_url(url, dest_path=None):
    if '/file/' in url:
        ...
        path = f'{file_id}!{url[re.search(file_id,
                                          url).end() + 1:]}'.split('!')
    elif '!' in url:
        path = re.findall(r'/#!(.*)', url)[0].split('!')
    else:
        raise Exception(translations["missing_url"])
