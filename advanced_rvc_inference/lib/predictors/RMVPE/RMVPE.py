...
class RMVPE:
    def __init__(
        ...
        self.mel_extractor = MelSpectrogram(
            N_MELS, 16000, 1024, 160, None, 30, 8000).to(device)
    ...
    def infer_from_audio(self, audio, thred=0.03):
        hidden = self.mel2hidden(
            self.mel_extractor(
                torch.from_numpy(audio).float().to(
                    self.device).unsqueeze(0),
                center=True))

        return self.decode(hidden.squeeze(0).cpu().numpy().astype(
            np.float32) if not self.onnx else hidden[0], thred=thred)
