...
class MODEL(torch.nn.Module):
    def __init__(self, model='full'):
        ...
        in_channels = {
            "full": [
                1, 1024, 128, 128, 128, 256], "large": [
                1, 768, 96, 96, 96, 192], "medium": [
                1, 512, 64, 64, 64, 128], "small": [
                    1, 256, 32, 32, 32, 64], "tiny": [
                        1, 128, 16, 16, 16, 32]}[model]
        out_channels = {
            "full": [
                1024, 128, 128, 128, 256, 512], "large": [
                768, 96, 96, 96, 192, 384], "medium": [
                512, 64, 64, 64, 128, 256], "small": [
                    256, 32, 32, 32, 64, 128], "tiny": [
                        128, 16, 16, 16, 32, 64]}[model]
        self.in_features = {
            "full": 2048,
            "large": 1536,
            "medium": 1024,
    ...
    def forward(self, x, embed=False):
        ...
        if embed:
            return x
        return self.classifier(self.layer(x, self.conv6, self.conv6_BN).permute(
            0, 2, 1, 3).reshape(-1, self.in_features)).sigmoid()
