...
def mean(signals, win_length=9):
    ...
    avg_pooled = torch.nn.functional.conv1d(
        ...
        ones_kernel,
        stride=1,
        padding=padding) / torch.nn.functional.conv1d(
            mask.float(),
            ones_kernel,
            stride=1,
...
def median(signals, win_length):
    ...
    mask = torch.nn.functional.pad(
        mask.float(), (padding, padding), mode="constant", value=0)
    ...
    x_sorted, _ = torch.where(
        mask.bool(), x.float(), float("inf")).to(x).sort(
        dim=-1)

    median_pooled = x_sorted.gather(-1, ((mask.sum(dim=-1) - 1) //
                                    2).clamp(min=0).unsqueeze(-1).long()).squeeze(-1)
