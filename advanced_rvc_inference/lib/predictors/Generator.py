from .WORLD.SWIPE import stonemask, swipe
from .CREPE.filter import mean, median
from ..utils import autotune_f0, circular_write, proposal_f0_up_key
from assets.config.variables import config, configs, logger, translations
from scipy.signal import medfilt
from librosa import piptrack, pyin, yin
import numpy as np
import math
import os
import re
...
def post_process(
    ...
    if manual_f0 is not None:
        ...
        f0[manual_x_pad *
           tf0: manual_x_pad *
           tf0 +
           len(replace_f0)] = replace_f0[:f0[manual_x_pad *
                                             tf0: manual_x_pad *
                                             tf0 +
                                             len(replace_f0)].shape[0]]
...
def realtime_post_process(
    ...
    f0_mel = torch.clip((f0_mel - f0_mel_min) * 254 /
                        (f0_mel_max - f0_mel_min) + 1, 1, 255, out=f0_mel)
...
class Generator:
    ...
    def _ensure_model_available(self, method, model_suffix=""):
        ...
        if self.auto_download_models:
            ...
            downloaded_path = ensure_f0_model_available(
                internal_method, auto_download=True)
    ...
    def calculator(
        ...
        if "hybrid" in f0_method:
            logger.debug(
                translations["hybrid_calc"].format(
                    f0_method=f0_method))
        ...
        f0 = model(
            ...
            x,
            p_len,
            filter_radius if filter_radius %
            2 != 0 else filter_radius +
            1)

        if proposal_pitch:
            up_key = proposal_f0_up_key(
                f0, proposal_pitch_threshold, configs["limit_f0"])
    ...
    def compute_f0(self, f0_method, x, p_len, filter_radius):
        if "pm" in f0_method:
            ...
            f0 = self.get_f0_mangio_crepe(
                x, p_len, split_f0[2]) if split_f0[0] == "mangio" else self.get_f0_crepe(
                x, p_len, split_f0[1], filter_radius=filter_radius)
            ...
            f0 = self.get_f0_mangio_penn(x, p_len) if f0_method.split(
                "-")[0] == "mangio" else self.get_f0_penn(x, p_len, filter_radius=filter_radius)
    ...
    def get_f0_hybrid(self, methods_str, x, p_len, filter_radius):
        ...
        f0_mix[voiced_mask] = np.exp(np.nansum(
            np.log(stacked + 1e-6) * weights[:, None], axis=0)[voiced_mask])
    ...
    def get_f0_crepe(self, x, p_len, model="full", filter_radius=3):
        if not hasattr(self, "crepe"):
            ...
            if not self._ensure_model_available("crepe"):
                raise FileNotFoundError(
                    "CREPE model not available. Please enable auto-download or manually download the model.")
        ...
        f0, pd = self.crepe.compute_f0(torch.tensor(np.copy(x))[
                                       None].float(), pad=True)
        if self.f0_onnx_mode and self.del_onnx_model:
            del self.crepe.model, self.crepe
    ...
    def get_f0_fcpe(
        ...
        if not hasattr(self, "fcpe"):
            ...
            if not self._ensure_model_available("fcpe"):
                raise FileNotFoundError(
                    "FCPE model not available. Please enable auto-download or manually download the model.")
            ...
            self.fcpe = FCPE(
                ...
                os.path.join(
                    configs["predictors_path"],
                    ("fcpe_legacy" if legacy else (
                        "fcpe" if previous else "ddsp_200k")) +
                    (
                        ".onnx" if self.f0_onnx_mode else ".pt")),
                hop_length=self.hop_length,
                f0_min=self.f0_min,
                f0_max=self.f0_max,
                dtype=torch.float32,
        ...
        if self.f0_onnx_mode and self.del_onnx_model:
            del self.fcpe.fcpe.model, self.fcpe
    ...
    def get_f0_rmvpe(self, x, p_len, clipping=False, filter_radius=3):
        if not hasattr(self, "rmvpe"):
            ...
            if not self._ensure_model_available("rmvpe"):
                raise FileNotFoundError(
                    "RMVPE model not available. Please enable auto-download or manually download the model.")
        ...
        f0 = self.rmvpe.infer_from_audio_with_pitch(
            x,
            thred=filter_radius,
            f0_min=self.f0_min,
            f0_max=self.f0_max) if clipping else self.rmvpe.infer_from_audio(
            x,
            thred=filter_radius)

        if self.f0_onnx_mode and self.del_onnx_model:
            del self.rmvpe.model, self.rmvpe
        return self._resize_f0(f0, p_len)
    ...
    def get_f0_pyworld(self, x, p_len, filter_radius, model="harvest"):
        if not hasattr(self, "pw"):
            ...
            self.pw = PYWORLD(
                os.path.join(
                    configs["predictors_path"], "world"), os.path.join(
                    configs["binary_path"], "world.bin"))
    ...
    def get_f0_djcm(self, x, p_len, clipping=False, filter_radius=3):
        if not hasattr(self, "djcm"):
            ...
            if not self._ensure_model_available("djcm"):
                raise FileNotFoundError(
                    "DJCM model not available. Please enable auto-download or manually download the model.")
        ...
        f0 = self.djcm.infer_from_audio_with_pitch(
            ...
            thred=filter_radius,
            f0_min=self.f0_min,
            f0_max=self.f0_max) if clipping else self.djcm.infer_from_audio(
            x,
            thred=filter_radius)
