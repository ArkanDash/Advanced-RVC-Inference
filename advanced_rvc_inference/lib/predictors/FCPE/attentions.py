...
def pad_to_multiple(tensor, multiple, dim=-1, value=0):
    ...
    if m.is_integer():
        return False, tensor
    return True, F.pad(tensor, (*((0,) * (-1 - dim) * 2), 0,
                       (math.ceil(m) * multiple - seqlen)), value=value)
...
def look_around(x, backward=1, forward=0, pad_value=-1, dim=2):
    ...
    return torch.cat([padded_x[:, ind:(ind + t), ...]
                     for ind in range(forward + backward + 1)], dim=dim)
...
def softmax_kernel(
    ...
    data_dash = torch.einsum(
        ...
        repeat(
            ...
            b=b,
            h=h).type_as(data))
    diag_data = (((data**2).sum(dim=-1) / 2.0) *
                 (data_normalizer**2)).unsqueeze(dim=-1)

    return (ratio *
            ((data_dash -
              diag_data -
              data_dash.max(dim=-
                            1, keepdim=True).values).exp() +
             eps) if is_query else ratio *
            ((data_dash -
              diag_data +
              eps).exp())).type_as(data)
...
class LocalAttention(nn.Module):
    def __init__(
        ...
        if use_rotary_pos_emb and (exists(rel_pos_emb_config) or exists(dim)):
            ...
            self.rel_pos = SinusoidalEmbeddings(
                dim, use_xpos=use_xpos, scale_base=default(
                    xpos_scale_base, window_size // 2))
    ...
    def forward(
        ...
        _, autopad, pad_value, window_size, causal, look_backward, look_forward, shared_qk = q.shape, self.autopad, - \
        ...
        if autopad:
            ...
            (_, q), (_, k), (_, v) = map(lambda t: pad_to_multiple(
        ...
        if shared_qk:
            k = F.normalize(k, dim=-1).type(k.dtype)
        ...
        if causal:
            ...
            if self.exact_windowsize:
                causal_mask = causal_mask | (
                    bq_t > (bq_k + (self.window_size * self.look_backward)))
        ...
        sim = sim.masked_fill(
            ((bq_k -
              (
                  self.window_size *
                  self.look_forward)) > bq_t) | (
                bq_t > (
                    bq_k +
                    (
                        self.window_size *
                        self.look_backward))) | pad_mask,
            mask_value) if not causal and self.exact_windowsize else sim.masked_fill(
            pad_mask,
            mask_value)
        ...
        if exists(mask):
            ...
            mask = repeat(rearrange(look_around(rearrange(mask,
                                                          '... (w n) -> (...) w n',
                                                          w=windows,
                                                          n=window_size),
                                                **{**look_around_kwargs,
...
class FastAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        if self.no_projection:
            q, k = q.softmax(dim=-1), (k.exp()
                                       if self.causal else k.softmax(dim=-2))
        else:
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        context_mask = default(context_mask,
                               mask) if not cross_attend else context_mask

        q, k, v = map(lambda t: rearrange(t, "b n (h d) -> b h n d", h=h),
                      (self.to_q(x), self.to_k(context), self.to_v(context)))
        (q, lq), (k, lk), (v, lv) = map(
        ...
        if not empty(q):
            if exists(context_mask):
                v.masked_fill_(~context_mask[:, None, :, None], 0.0)
            if cross_attend:
                pass
            else:
