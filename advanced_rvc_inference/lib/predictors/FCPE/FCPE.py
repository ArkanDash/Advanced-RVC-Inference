...
from main.library.predictors.FCPE.utils import (
    DotDict, batch_interp_with_replacement_detach, decrypt_model,
    l2_regularization)
from main.library.predictors.FCPE.encoder import (ConformerNaiveEncoder,
                                                  EncoderLayer)
import os
import sys
...
@torch.no_grad()
def latent2cents_local_decoder(
        cent_table,
        out_dims,
        y,
        threshold=0.05,
    ...
    local_argmax_index = torch.arange(0, 9).to(
        max_index.device) + (max_index - 4)
    local_argmax_index[local_argmax_index < 0] = 0
    local_argmax_index[local_argmax_index >= out_dims] = out_dims - 1

    y_l = y.gather(-1, local_argmax_index)
    rtn = (ci.gather(-1, local_argmax_index) * y_l).sum(dim=-
                                                        1, keepdim=True) / y_l.sum(dim=-1, keepdim=True)
...
def cents_decoder(cent_table, y, confidence, threshold=0.05, mask=True):
    ...
    B, N, _ = y.size()
    rtn = (cent_table[None, None, :].expand(B, N, -1) *
           y).sum(dim=-1, keepdim=True) / y.sum(dim=-1, keepdim=True)
...
def cents_local_decoder(
    ...
    confident, max_index = y.max(dim=-1, keepdim=True)
    local_argmax_index = (torch.arange(0, 9).to(
        max_index.device) + (max_index - 4)).clamp(0, n_out - 1)
    y_l = y.gather(-1, local_argmax_index)
    rtn = (cent_table[None, None, :].expand(B, N, -1).gather(-1, local_argmax_index)
           * y_l).sum(dim=-1, keepdim=True) / y_l.sum(dim=-1, keepdim=True)
...
class PCmer(nn.Module):
    def __init__(
        ...
        self._layers = nn.ModuleList(
            [EncoderLayer(self) for _ in range(num_layers)])
...
class CFNaiveMelPE(nn.Module):
    def __init__(
        ...
        self.harmonic_emb = nn.Embedding(
            9, hidden_dims) if use_harmonic_emb else None
        self.input_stack = nn.Sequential(
            nn.Conv1d(
                input_channels, hidden_dims, 3, 1, 1), nn.GroupNorm(
                4, hidden_dims), nn.LeakyReLU(), nn.Conv1d(
                hidden_dims, hidden_dims, 3, 1, 1))
        ...
        self.cent_table_b = torch.linspace(
            f0_to_cent(
                torch.Tensor(
                    [f0_min]))[0], f0_to_cent(
                torch.Tensor(
                    [f0_max]))[0], out_dims).detach()
        self.register_buffer("cent_table", self.cent_table_b)
        self.gaussian_blurred_cent_mask_b = (
            1200 * torch.Tensor([self.f0_max / 10.]).log2())[0].detach()
    ...
    def forward(self, x, _h_emb=None):
        ...
        if self.harmonic_emb is not None:
            x = x + self.harmonic_emb(torch.LongTensor([0]).to(
                x.device)) if _h_emb is None else x + self.harmonic_emb(torch.LongTensor([int(_h_emb)]).to(x.device))
    ...
    def infer(self, mel, decoder="local_argmax", threshold=0.05):
        ...
        return cent_to_f0(
            latent2cents_decoder(
                self.cent_table,
                latent,
                threshold=threshold) if decoder == "argmax" else latent2cents_local_decoder(
                self.cent_table,
                self.out_dims,
                latent,
                threshold=threshold))
...
class FCPE_LEGACY(nn.Module):
    def __init__(
        ...
        self.stack = nn.Sequential(
            nn.Conv1d(
                input_channel, n_chans, 3, 1, 1), nn.GroupNorm(
                4, n_chans), nn.LeakyReLU(), nn.Conv1d(
                n_chans, n_chans, 3, 1, 1))
        self.decoder = PCmer(
            num_layers=n_layers,
            num_heads=8,
            dim_model=n_chans,
                ...
                self.decoder(
                    (self.stack(
                        mel.transpose(
                            ...
                            2)).transpose(
                        1,
                        2) if self.use_input_conv else mel)))).sigmoid()

        if not infer:
            loss_all = self.loss_mse_scale * \
                F.binary_cross_entropy(x, self.gaussian_blurred_cent(f0_to_cent(gt_f0)))
            if self.loss_l2_regularization:
                loss_all = loss_all + \
                    l2_regularization(model=self, l2_alpha=self.loss_l2_regularization_scale)
            x = loss_all
        else:
            x = cent_to_f0(
                cents_decoder(
                    ...
                    mask=True) if cdecoder == "argmax" else cents_local_decoder(
                    self.cent_table,
                    x,
                    self.n_out,
                    self.confidence,
    ...
    def gaussian_blurred_cent(self, cents):
        B, N, _ = cents.size()
        return (-(self.cent_table[None, None, :].expand(B, N, -1) - cents).square() / 1250).exp(
        ) * (cents > 0.1) & (cents < (1200.0 * np.log2(self.f0_max / 10.0))).float()
    ...
    def infer(
        ...
        if interp_uv:
            f0 = batch_interp_with_replacement_detach(
                uv.squeeze(-1).bool(), f0.squeeze(-1)).unsqueeze(-1)
        ...
        if return_uv:
            return f0, F.interpolate(uv.transpose(1, 2), size=int(
                output_interp_target_length), mode="nearest").transpose(1, 2)
        else:
            return f0
...
class FCPEInfer_LEGACY:
    ...
    def __call__(self, audio, sr, threshold=0.05, p_len=None):
        ...
        if not hasattr(self, "numpy_threshold") and self.onnx:
            self.numpy_threshold = np.array(threshold, dtype=np.float32)
