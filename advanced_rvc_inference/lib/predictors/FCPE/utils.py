...
def decrypt_model(configs, input_path):
    ...
    return BytesIO(unpad(AES.new(key, AES.MODE_CBC, data[:16]).decrypt(
        data[16:]), AES.block_size)).read()
...
def torch_interp(x, xp, fp):
    ...
    interp_vals = y_left + \
        ((x - x_left) * (fp[right_idxs] - y_left) / (xp[right_idxs] - x_left))
...
def batch_interp_with_replacement_detach(uv, f0):
    ...
    for i in range(uv.shape[0]):
        interp_vals = torch_interp(torch.where(
            uv[i])[-1], torch.where(~uv[i])[-1], f0[i][~uv[i]]).detach()
