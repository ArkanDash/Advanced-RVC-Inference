from main.library.predictors.DJCM.utils import (N_CLASS, SAMPLE_RATE,
                                                WINDOW_LENGTH)
...
class DJCM:
    def __init__(
        ...
        self.spec_extractor = Spectrogram(
            int(SAMPLE_RATE // 100), WINDOW_LENGTH).to(device)
            ...
            hidden = torch.as_tensor(
                self.model.run(
                    [
                        ...
                        self.model.get_inputs()[0].name: spec.cpu().numpy().astype(
                            np.float32)})[0], device=self.device)
        else:
            hidden = self.model(
    ...
    def infer_from_audio(self, audio, thred=0.03):
        ...
        with torch.no_grad():
            ...
            hidden = self.inference(padded_audio)[:(
                audio.shape[-1] // int(SAMPLE_RATE // 100) + 1)]
    ...
    def infer_from_audio_with_pitch(
            self,
            audio,
            thred=0.03,
            f0_min=50,
    ...
    def to_local_average_cents(self, salience, thred=0.05):
        ...
        for idx in range(salience.shape[0]):
            ...
            todo_cents_mapping.append(
                self.cents_mapping[starts[idx]: ends[idx]])

        todo_salience = np.array(todo_salience)
        devided = np.sum(
            todo_salience * np.array(todo_cents_mapping), 1) / np.sum(todo_salience, 1)
    ...
    def pad_audio(self, audio):
        ...
        right_pad = np.zeros(
            int(pad_len - self.seg_len // 4), dtype=np.float32)
        padded_audio = np.concatenate([left_pad, audio, right_pad], axis=-1)

        segments = [padded_audio[start: start + int(self.seg_len)] for start in range(
            0, len(padded_audio) - int(self.seg_len) + 1, int(self.seg_len // 2))]
