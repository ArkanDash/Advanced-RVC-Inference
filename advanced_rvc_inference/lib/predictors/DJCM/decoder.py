from main.library.predictors.DJCM.utils import (N_CLASS, WINDOW_LENGTH, BiGRU,
                                                ResConvBlock, init_bn,
                                                init_layer)
...
class ResDecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, n_blocks, stride):
        ...
        self.conv1 = nn.ConvTranspose2d(
            in_channels, out_channels, stride, stride, (0, 0), bias=False)
        self.bn1 = nn.BatchNorm2d(in_channels, momentum=0.01)
        self.conv = nn.ModuleList(
            [ResConvBlock(out_channels * 2, out_channels)])
