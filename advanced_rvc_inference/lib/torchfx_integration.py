...
class TorchFXProcessor:
    ...
    def __init__(self, device: Optional[Union[str, torch.device]] = None):
        ...
        if self._is_torchfx_available:
            logger.info(
                f"TorchFX initialized successfully on device: {
                    self.device}")
        else:
    ...
    def process_audio_with_torchfx(
            self,
...
def get_torchfx_processor(
        device: Optional[Union[str, torch.device]] = None) -> TorchFXProcessor:
