...
class TorchAudioMentationsProcessor:
    ...
    def __init__(self, sample_rate: int = 44100,
        ...
        if self._is_available:
            ...
            logger.info(
                f"TorchAudioMentations initialized on device: {
                    self.device}")
        else:
    ...
    def _check_availability(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"TorchAudioMentations components not fully available: {e}")
            return False
    ...
    def _initialize_transforms(self):
        ...
        try:
            ...
            logger.info(
                f"Initialized {len(self.transforms)} augmentation transforms")
    ...
    def augment_audio_batch(self, audio_batch: torch.Tensor,
        ...
        if not self._is_available:
            logger.info(
                "TorchAudioMentations not available, skipping augmentation")
            return audio_batch
...
class RVCAudioAugmenter:
    ...
    def apply_preset(
        ...
        return self.processor.augment_audio_batch(
            audio, transforms, preset['probability'])
...
def get_audio_mentations_processor(sample_rate: int = 44100,
                                   device: Optional[Union[str,
                                                          torch.device]] = None) -> TorchAudioMentationsProcessor:
    ...
    if audio_mentations_processor is None:
        audio_mentations_processor = TorchAudioMentationsProcessor(
            sample_rate, device)
...
def get_rvc_augmenter(sample_rate: int = 44100,
                      device: Optional[Union[str,
                                             torch.device]] = None) -> RVCAudioAugmenter:
