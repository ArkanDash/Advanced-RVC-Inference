...
from .torch_audiomentations_integration import (RVCAudioAugmenter,
                                                TorchAudioMentationsProcessor)
from .torch_compile_optimization import TorchCompileOptimizer, compile_function
from .torchfx_integration import (TorchFXMelSpectrogramProcessor,
                                  TorchFXProcessor)
...
class UnifiedPerformanceOptimizer:
        ...
        try:
            if self.enable_torchfx:
                ...
                if self.torchfx_processor._is_torchfx_available:
                    ...
                    logger.warning(
                        "TorchFX not available, DSP optimization disabled")
                    self.enable_torchfx = False

            if self.enable_augmentation:
                self.audio_augmenter = RVCAudioAugmenter(
                ...
                if self.audio_augmenter.processor._is_available:
                    ...
                    logger.warning(
                        "Torch-audiomentations not available, augmentation disabled")
            ...
            if self.enable_compilation:
                ...
                if self.compile_optimizer.compile_available:
                    ...
                    logger.warning(
                        "torch.compile not available, JIT optimization disabled")
    ...
    def process_audio_batch(self,
        ...
        if self.enable_torchfx and self.torchfx_processor:
            try:
                dsp_pipeline = self.torchfx_processor.create_audio_pipeline(
                    dsp_filters)
        ...
        if self.enable_augmentation and self.audio_augmenter and augmentation_preset:
            try:
                if augmentation_preset in self.audio_augmenter.presets:
                    ...
                    logger.debug(
                        f"Applied augmentation preset: {augmentation_preset}")
            except Exception as e:
    ...
    def optimize_model(self,
        ...
        try:
            ...
            logger.info(
                f"Model optimized in {
                    optimization_time:.2f}s with {mode} mode")
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        if self.audio_augmenter:
            info['augmentation_info'] = {
                'presets': list(
                    self.audio_augmenter.presets.keys()),
...
class OptimizedRVCPipeline:
    ...
    def _build_pipeline(self):
        ...
        if self.torchfx_processor:
            dsp_filters = self.dsp_config.get(
                'filters', ['lowpass', 'highpass', 'normalize'])
            dsp_pipeline = self.torchfx_processor.create_audio_pipeline(
                dsp_filters)
            self.processors.append(('dsp', dsp_pipeline))

        # Add augmentation (training mode)
        if self.audio_augmenter and self.augmentation_config.get(
                'enable_training_augmentation', False):
        ...
        for processor_name, processor in self.processors:
            try:
                if processor_name == 'dsp':
                    ...
                    preset = self.augmentation_config.get(
                        'preset', 'voice_preservation')
                    processed = processor.apply_preset(processed, preset)

            except Exception as e:
                logger.warning(
                    f"Pipeline processing failed at {processor_name}: {e}")
                continue
...
def get_unified_optimizer(device: Optional[Union[str,
                                                 torch.device]] = None,
                          enable_torchfx: bool = True,
                          enable_augmentation: bool = True,
                          enable_compilation: bool = True,
...
def create_optimized_rvc_system(model: Optional[nn.Module] = None,
    ...
    if model:
        ...
        optimized_model = optimizer.optimize_model(
            model, example_input, optimization_level)
