    ...
    def _detect_gpu(self) -> Dict[str, Any]:
        ...
        if torch.cuda.is_available():
            ...
            for i in range(gpu_count):
                ...
                logger.info(
                    f"Memory: {
                        memory_gb:.1f}GB, Tensor Cores: {tensor_cores}")
...
class OpenCLAudioProcessor:
    ...
    def __init__(self, device_id: int = 0):
        ...
        try:
            ...
            self._load_kernels()
            logger.info(
                f"OpenCL initialized successfully on device {device_id}")
    ...
    def _filter_opencl(self, audio_data: np.ndarray) -> np.ndarray:
        ...
        output_buf = cl.Buffer(
            ...
            cl.mem_flags.WRITE_ONLY,
            output.nbytes)
        filter_buf = cl.Buffer(
            self.context,
            cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR,
            hostbuf=filter_coeffs)

        # Execute kernel
        self.program.audio_filter(
            self.queue,
            audio_data.shape,
            None,
            audio_buf,
...
def get_opencl_processor(device_id: int = 0) -> Optional[OpenCLAudioProcessor]:
    ...
    if cache_key not in get_opencl_processor._cache:
        try:
            get_opencl_processor._cache[cache_key] = OpenCLAudioProcessor(
                device_id)
        except Exception as e:
