...
from ...lib.infer.extracting.preparing_files import (generate_config,
                                                     generate_filelist)
...
def main():
    ...
    devices = ["cpu"] if gpus == "-" else [
        (f"cuda:{idx}" if config.device.startswith("cuda") else f"{
            'ocl' if config.device.startswith('ocl') else 'privateuseone'}:{idx}") for idx in gpus.split("-")]
