...
try:
    ...
    # Create fallback config, logger and translations if main module doesn't
    # exist
...
class FeatureInput:
    ...
    def process_file(
            ...
            try:
                ...
                class FallbackGenerator:
                    def calculator(
                        ...
                        # This is a simplified fallback - real implementation
                        # would be more complex
                        f0, _ = librosa.pyin(
                            x, fmin=self.f0_min, fmax=self.f0_max)
                        f0 = np.nan_to_num(f0)
        ...
        if os.path.exists(
                opt_path1 +
                ".npy") and os.path.exists(
                opt_path2 +
                ".npy"):
            return

        try:
            pitch, pitchf = self.f0_gen.calculator(x_pad=config.x_pad, f0_method=f0_method, x=load_audio(file_inp, self.sample_rate), f0_up_key=0, p_len=None,
                                                   filter_radius=3, f0_autotune=f0_autotune, f0_autotune_strength=f0_autotune_strength, manual_f0=None, proposal_pitch=False, proposal_pitch_threshold=0.0)
            np.save(opt_path2, pitchf, allow_pickle=False)
            np.save(opt_path1, pitch, allow_pickle=False)
        except Exception as e:
            logger.info(
                f"{translations['extract_file_error']} {inp_path}: {e}")
        ...
        with tqdm.tqdm(total=len(files), ncols=100, unit="p", leave=True) as pbar:
            with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
                for _ in concurrent.futures.as_completed(
                        [executor.submit(worker, f) for f in files]):
                    pbar.update(1)
...
def run_pitch_extraction(
    ...
    output_root1, output_root2 = output_roots if len(
        output_roots) == 2 else (output_roots[0], None)

    logger.info(translations["extract_f0_method"].format(
        num_processes=num_processes, f0_method=f0_method))
    num_processes = 1 if config.device.startswith(("ocl", "privateuseone")) and (
        "crepe" in f0_method or "fcpe" in f0_method or "rmvpe" in f0_method or "penn" in f0_method or "swift" in f0_method) else num_processes
    paths = [
        (os.path.join(
            input_root, name), os.path.join(
            output_root1, name) if output_root1 else None, os.path.join(
                output_root2, name) if output_root2 else None, os.path.join(
                    input_root, name)) for name in sorted(
                        os.listdir(input_root)) if "spec" not in name]

    start_time = time.time()
    feature_input = FeatureInput()
    with concurrent.futures.ProcessPoolExecutor(max_workers=len(devices)) as executor:
        concurrent.futures.wait([executor.submit(feature_input.process_files,
                                                 paths[i::len(devices)],
                                                 f0_method,
                                                 hop_length,
                                                 f0_onnx,
                                                 ...
                                                 f0_autotune,
                                                 f0_autotune_strength,
                                                 alpha) for i in range(len(devices))])

    gc.collect()
    logger.info(translations["extract_f0_success"].format(
