...
def process_file_rms(files, device, threads):
    ...
    def worker(file_info):
        try:
            ...
            if os.path.exists(out_file_path + ".npy"):
                return
            feats = torch.from_numpy(load_audio(file, 16000)).unsqueeze(0)

            with torch.no_grad():
                feats = module(
                    feats if device.startswith(
                        ("ocl", "privateuseone")) else feats.to(device))
...
def run_rms_extraction(exp_dir, num_processes, devices, rms_extract):
    if rms_extract:
        ...
        logger.info(
            translations["rms_start_extract"].format(
                num_processes=num_processes))

        with concurrent.futures.ProcessPoolExecutor(max_workers=len(devices)) as executor:
            concurrent.futures.wait([executor.submit(process_file_rms, paths[i::len(
                devices)], devices[i], num_processes // len(devices)) for i in range(len(devices))])

        logger.info(translations["rms_success_extract"].format(
