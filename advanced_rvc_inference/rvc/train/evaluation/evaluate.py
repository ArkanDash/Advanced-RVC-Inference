...
def calculate_audio_metrics(original_audio, converted_audio, sr=16000):
    ...
    try:
        ...
        original_centroid = librosa.feature.spectral_centroid(y=original, sr=sr)[
            0]
        converted_centroid = librosa.feature.spectral_centroid(y=converted, sr=sr)[
            0]
        metrics["spectral_centroid_diff"] = float(
            np.mean(np.abs(original_centroid - converted_centroid)))

        # 2. RMS energy difference
        original_rms = librosa.feature.rms(y=original)[0]
        converted_rms = librosa.feature.rms(y=converted)[0]
        metrics["rms_energy_diff"] = float(
            np.mean(np.abs(original_rms - converted_rms)))
        ...
        try:
            ...
            if len(original_f0) > 0 and len(converted_f0) > 0:
                ...
                correlation = np.corrcoef(
                    original_f0[:min_len_f0], converted_f0[:min_len_f0])[0, 1]
                metrics["pitch_correlation"] = float(
