...
def generate_config(sample_rate: int, model_path: str):
    config_path = os.path.join("advanced_rvc_inference", "rvc", "configs", f"{sample_rate}.json")
...
def generate_filelist(model_path: str, sample_rate: int, include_mutes: int = 2):
    ...
    if embedder_name == "spin":
        mute_base_path = os.path.join(current_directory, "advanced_rvc_inference", "logs", "mute_spin")
    elif embedder_name == "spin-v2":
        mute_base_path = os.path.join(current_directory, "advanced_rvc_inference", "logs", "mute_spin-v2")
    else:
        mute_base_path = os.path.join(current_directory, "advanced_rvc_inference", "logs", "mute")
