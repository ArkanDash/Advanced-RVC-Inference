...
def process_audio_files(
    ...
    if not preprocess_dir.exists():
        raise FileNotFoundError(
            f"Preprocessed data not found: {preprocess_dir}")
    ...
    if not audio_files:
        raise ValueError(
            f"No preprocessed audio files found in {preprocess_dir}")
...
def main():
    parser = argparse.ArgumentParser(
        description="RVC Feature and F0 Extraction")
    parser.add_argument(
        "--model_name",
        required=True,
