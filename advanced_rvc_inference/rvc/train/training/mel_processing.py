...
def spectrogram_torch(y, n_fft, hop_size, win_size, center=False):
    ...
    wnsize_dtype_device = str(win_size) + "_" + \
        str(y.dtype) + "_" + str(y.device)
    if wnsize_dtype_device not in hann_window:
        hann_window[wnsize_dtype_device] = torch.hann_window(
            win_size).to(dtype=y.dtype, device=y.device)
    pad = torch.nn.functional.pad(y.unsqueeze(1),
    ...
    if str(y.device).startswith(("ocl", "privateuseone")):
        ...
        spec = torch.stft(
            ...
            hop_length=hop_size,
            win_length=win_size,
            window=hann_window[wnsize_dtype_device].to(
                pad.device),
            center=center,
            pad_mode="reflect",
            normalized=False,
...
def spec_to_mel_torch(spec, n_fft, num_mels, sample_rate, fmin, fmax):
    ...
    fmax_dtype_device = str(fmax) + "_" + \
        str(spec.dtype) + "_" + str(spec.device)
...
class MultiScaleMelSpectrogramLoss(torch.nn.Module):
    def __init__(
        ...
        self.hann_window = {}
        self.mel_banks = {}
        self.stft_params = [(mel, win)
                            for mel, win in zip(n_mels, window_lengths)]
    ...
    def mel_spectrogram(self, wav, n_mels, window_length):
        ...
        if str(wav.device).startswith(("ocl", "privateuseone")):
            stft = torch.stft(
                ...
                return_complex=True)
            magnitude = (stft.real.pow(2) + stft.imag.pow(2) +
                         1e-6).sqrt().to(wav.device, dtype=torch.float32)
        else:
            stft = torch.stft(
                wav,
                n_fft=window_length,
    ...
    def forward(self, real, fake):
        ...
        for p in self.stft_params:
            loss += self.loss_fn(self.mel_spectrogram(real,
                                                      *p).clamp(min=1e-5).log() / self.log_base,
                                 self.mel_spectrogram(fake,
                                                      *p).clamp(min=1e-5).log() / self.log_base)
        return loss
