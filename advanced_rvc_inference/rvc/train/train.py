...
experiment_dir = os.path.join(current_dir, "advanced_rvc_inference", "logs", model_name)
...
def main():
    ...
    if cleanup:
        ...
        for root, dirs, files in os.walk(
            os.path.join("advanced_rvc_inference", "logs", model_name), topdown=False
        ):
            for name in files:
...
def run(
    ...
    if os.path.isfile(os.path.join("advanced_rvc_inference", "logs", "reference", embedder_name, "feats.npy")):
        print("Using", embedder_name, "reference set for validation")
        phone = np.load(os.path.join("advanced_rvc_inference", "logs", "reference", embedder_name, "feats.npy"))
        ...
        pitch = np.load(os.path.join("advanced_rvc_inference", "logs", "reference", "pitch_coarse.npy"))
        # removed last frame to match features
        pitch = torch.LongTensor(pitch[:-1]).unsqueeze(0).to(device)
        pitchf = np.load(os.path.join("advanced_rvc_inference", "logs", "reference", "pitch_fine.npy"))
        # removed last frame to match features
