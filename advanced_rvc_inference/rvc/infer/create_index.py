...
from assets.config.variables import configs, logger, translations
import argparse
import os
import sys
...
def main():
    ...
    try:
        ...
        n_ivf = min(
            int(16 * np.sqrt(big_npy.shape[0])), big_npy.shape[0] // 39)
        ...
        faiss.write_index(
            index_trained, os.path.join(
                exp_dir, f"trained_IVF{n_ivf}_Flat_nprobe_{
                    index_ivf_trained.nprobe}_{model_name}_{version}.index"))
        ...
        index_filepath_added = os.path.join(
            exp_dir, f"added_IVF{n_ivf}_Flat_nprobe_{
                index_ivf_added.nprobe}_{model_name}_{version}.index")
...
def create_index(model_name, rvc_version="v2", index_algorithm="Auto"):
    ...
    try:
        ...
        logger.info(
            f"Creating index for model '{model_name}' with version {version} using {algorithm} algorithm")
        ...
        index_filepath = os.path.join(
            exp_dir, f"index_{model_name}_{version}.index")
        os.makedirs(exp_dir, exist_ok=True)
        faiss.write_index(index, index_filepath)

        logger.info(
            f"{translations.get('save_index', 'Saved index')} '{index_filepath}'")
        return index_filepath

    except Exception as e:
        logger.error(
            f"{translations.get('create_index_error', 'Error creating index')}: {e}")
        import traceback
        logger.debug(traceback.format_exc())
        raise
