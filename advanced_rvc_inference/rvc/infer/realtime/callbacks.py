from ...lib.infer.realtime.realtime import RVC_Realtime, VoiceChanger
from ...lib.infer.realtime.audio import Audio
...
class AudioCallbacks:
    ...
    def change_voice(
        ...
        if self.pass_through:
            vol = float(
                np.sqrt(
                    np.square(received_data).mean(
                        dtype=np.float32)))
            return received_data, vol, [0, 0, 0], None

        try:
            with self.lock:
                audio, vol, perf = self.vc.on_request(received_data, f0_up_key, index_rate, protect, filter_radius,
