from ...lib.utils import check_assets, circular_write
...
class RVC_Realtime:
    ...
    def initialize(self):
        ...
        if self.clean_audio:
            ...
            self.tg = TorchGate(
                self.sample_rate,
                prop_decrease=self.clean_strength).to(
                config.device)
        else:
            self.tg = None
    ...
    def realloc(
        ...
        convert_size_16k = block_frame_16k + sola_search_frame_16k + \
            extra_frame_16k + crossfade_frame_16k
        ...
        self.silence_front = extra_frame_16k - \
            (self.window_size * 5) if self.silence_front else 0
        ...
        self.convert_buffer = torch.zeros(
            convert_size_16k, dtype=self.dtype, device=config.device)
    ...
    def inference(
        ...
        if self.tg is not None:
            audio_model = self.tg(audio_model.unsqueeze(0)).squeeze(0)
...
class VoiceChanger:
    ...
    def process_audio(
        ...
        audio, vol = self.vc_model.inference(audio_in, f0_up_key, index_rate, protect, filter_radius,
                                             rms_mix_rate, f0_autotune, f0_autotune_strength, proposal_pitch, proposal_pitch_threshold)

        if audio is None:
            return np.zeros(block_size, dtype=np.float32), vol

        conv_input = audio[None, None,
        ...
        audio[: self.crossfade_frame] += (self.sola_buffer *
                                          self.fade_out_window)

        self.sola_buffer[:] = audio[block_size: block_size +
                                    self.crossfade_frame]
    ...
    def on_request(
        ...
        result, vol = self.process_audio(audio_in, f0_up_key, index_rate, protect, filter_radius,
                                         rms_mix_rate, f0_autotune, f0_autotune_strength, proposal_pitch, proposal_pitch_threshold)
