...
def create_reference(
    ...
    if os.path.exists(output_reference):
        shutil.rmtree(reference_name, ignore_errors=True)
    ...
    with tqdm(total=5, desc=translations["create_reference"], ncols=100, unit="a") as pbar:
        ...
        if isinstance(embedder, torch.nn.Module):
            embedder = embedder.to(
                torch.float16 if is_half else torch.float32).eval().to(device)

        with torch.no_grad():
            feats = extract_features(
                embedder, audio_pad.view(
                    1, -1), version, device=device)
        ...
        if use_energy:
            ...
            rms = RMSEnergyExtractor(
                ...
                hop_length=HOP_SIZE,
                center=True,
                pad_mode="reflect").to(device).eval()
    ...
    logger.info(translations["create_reference_success"].format(
