...
def create_dataset(
    ...
    try:
        ...
        if skip_seconds:
            skip_start_audios, skip_end_audios = skip_start_audios.replace(
                ", ", ",").split(","), skip_end_audios.replace(", ", ",").split(",")

            if len(skip_start_audios) < len(audio_path) or len(
                    skip_end_audios) < len(audio_path):
        ...
        for audio in audio_path:
            ...
            if sr != sample_rate:
                data = librosa.resample(
                    ...
                    res_type="soxr_vhq")
            if clean_dataset:
                data = tg(torch.from_numpy(data).unsqueeze(0).to(
                    config.device).float()).squeeze(0).cpu().detach().numpy()
    ...
    logger.info(
        translations["create_dataset_success"].format(
            elapsed_time=f"{
                elapsed_time:.2f}"))
    return output_dirs
...
def skip_start(
    ...
    if seconds <= 0:
        ...
        logger.warning(
            translations["skip_warning"].format(
                seconds=seconds,
                total_duration=f"{
                    total_duration:.2f}"))
    else:
        ...
        logger.info(
            translations["skip_start_audio"].format(
...
def skip_end(
    ...
    if seconds <= 0:
        ...
        logger.info(
            translations["skip_end_audio"].format(
                input_file=input_file))
