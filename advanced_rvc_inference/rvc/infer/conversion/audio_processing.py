...
def automatic_multiband_eq(
    ...
    for i in range(n_bands):
        ...
        if low <= 0:
            ...
            b, a = signal.butter(
                2, [low / (sr / 2), high / (sr / 2)], btype="band")
...
def best_multiband_eq(
    ...
    if original_audio is not None:
        ...
        bands = [(band_edges[i], band_edges[i + 1], gains_db[i])
                 for i in range(n_bands)]
        eq_audio = apply_multiband_eq(audio, sr, bands)
...
def spectral_subtract_denoise(
    ...
    if stft is None:
        fft = torch.stft(
            ...
            window=window,
            return_complex=True)
        mag, phase = (fft.real.pow(2) + fft.imag.pow(2)
                      ).sqrt(), fft.imag.data.atan2(fft.real.data)
    else:
    ...
    noise_mag = mag[:,
                    :,
                    :max(1,
                         min(int((noise_seconds * sr - n_fft) // hop_length) + 1,
                             mag.shape[-1]))].mean(dim=-1,
                                                   keepdim=True)
    clean_mag = (mag - alpha * noise_mag).maximum((noise_mag * 1.0) * 0.1)

    xrec = torch.istft(clean_mag * (1j * phase).exp(),
                       n_fft=n_fft,
                       hop_length=hop_length,
                       win_length=n_fft,
                       window=window,
                       length=x.shape[0]) if stft is None else stft.inverse(clean_mag,
                                                                            phase)
...
def repair_bad_frames(audio, sr, frame_ms=20, energy_thresh=0.02):
    ...
    frames = np.stack([audio[i * hop: i * hop + frame_len]
    ...
    for i, is_bad in enumerate(bad):
        ...
        if left.size > 0 and right.size > 0:
            out[start:end] = 0.5 * (np.mean(left) + np.mean(right))
        elif left.size > 0:
            out[start:end] = left[-1]
        elif right.size > 0:
            out[start:end] = right[0]
        else:
            out[start:end] = 0.0
...
def harmonic_enrich_and_compress(
        audio,
        drive=0.02,
        comp_ratio=3.0,
        frame_length=1024,
