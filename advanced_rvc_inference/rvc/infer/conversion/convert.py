...
from .audio_processing import postprocess, preprocess
from ...utils import (check_assets, clear_gpu_cache, cut, load_audio,
                      load_embedders_model, load_model, restore)
            ...
            with tqdm(total=10, desc=translations["convert_audio"], ncols=100, unit="a", leave=not split_audio) as pbar:
                ...
                if not self.hubert_model:
                    ...
                    if isinstance(models, torch.nn.Module):
                        import torch
                        models = models.to(
                            torch.float16 if self.config.is_half else torch.float32).eval().to(
                            self.device)
                ...
                if split_audio:
                    ...
                    logger.info(
                        f"{translations['split_total']}: {len(chunks)}")
                    pbar = tqdm(
                        total=len(chunks) * 5 + 4,
                        desc=translations["convert_audio"],
                        ncols=100,
                        unit="a",
                        leave=True)
                else:
                ...
                audio_output = restore(
                    converted_chunks,
                    total_len=len(audio),
                    dtype=converted_chunks[0][2].dtype) if split_audio else converted_chunks[0][2]
                ...
                if len(audio) / self.sample_rate > len(audio_output) / \
                        self.tgt_sr:
                    padding = np.zeros(int(np.round(len(audio) /
                                                    self.sample_rate *
                                                    self.tgt_sr) -
                                           len(audio_output)), dtype=audio_output.dtype)
        ...
        if self.cpt is not None:
            if self.loaded_model.endswith(".pth"):
                ...
                self.net_g = self.net_g.to(
                    torch.float16 if self.config.is_half else torch.float32)
...
def run_convert_script(
    ...
    if output_path is None:
        input_filename = os.path.splitext(os.path.basename(input_path))[
            0] if input_path else "output"
        output_path = os.path.join(
            str(path('outputs_dir')), f"{input_filename}_converted.{export_format}")
    ...
    log_data = {
        ...
        translations['output_path']: replace_export_format(
            output_path,
            export_format),
    ...
    if resample_sr != 0:
        log_data[translations['sample_rate']] = resample_sr
    if f0_autotune:
        log_data[translations['autotune_rate_info']] = f0_autotune_strength
    if os.path.isfile(f0_file):
        log_data[translations['f0_file']] = f0_file
    if proposal_pitch:
        log_data[translations["proposal_pitch_threshold"]
                 ] = proposal_pitch_threshold
    if formant_shifting:
    ...
    if not pth_path or not os.path.exists(pth_path) or os.path.isdir(
            pth_path) or not pth_path.endswith((".pth", ".onnx")):
        logger.warning(
            translations["provide_file"].format(
                filename=translations["model"]))
        sys.exit(1)
    ...
    if os.path.isdir(input_path):
        ...
        audio_files = [
            f for f in os.listdir(input_path) if f.lower().endswith(
                ("wav",
                 "mp3",
                 "flac",
                 "ogg",
        ...
        logger.info(
            translations["found_audio"].format(
            ...
            translations["convert_batch_success"].format(
                elapsed_time=f"{
                    (
                        time.time() -
                        start_time):.2f}",
                output_path=replace_export_format(
                    output_path,
                    export_format)))
    else:
