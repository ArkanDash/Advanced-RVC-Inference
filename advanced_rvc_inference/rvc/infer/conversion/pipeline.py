from ....lib.utils import (autotune_f0, change_rms, clear_gpu_cache,
                           extract_features, extract_median_f0,
                           load_faiss_index, proposal_f0_up_key)
...
class Pipeline:
    ...
    def voice_conversion(
        ...
        rich_logger.debug(
            f"Voice conversion - Pitch guidance: {pitch_guidance}, Energy: {energy_use}")

        # Prepare audio features
        feats = torch.from_numpy(audio0).to(self.device).to(
        ...
        with torch.no_grad():
            ...
            if index is not None and big_npy is not None and index_rate != 0:
                rich_logger.debug(
                    f"Applying index enhancement (rate: {index_rate})...")
                ...
                feats = (torch.from_numpy(npy).unsqueeze(0).to(
                    self.device) * index_rate + (1 - index_rate) * feats)

            # Resize features for upsampling
            feats = F.interpolate(
                feats.permute(
                    0, 2, 1), scale_factor=2).permute(
                0, 2, 1)
            ...
            if feats0 is not None:
                ...
                feats0 = F.interpolate(
                    feats0.permute(
                        0, 2, 1), scale_factor=2).permute(
                    0, 2, 1)
                feats = (feats * pitchff + feats0 *
                         (1 - pitchff)).to(feats0.dtype)
    ...
    def pipeline(
        ...
        try:
            rich_logger.debug(
                f"Pipeline conversion started - Method: {f0_method}, Pitch guidance: {pitch_guidance}")

            # Load FAISS index if needed
            index, big_npy = load_faiss_index(
                file_index) if index_rate != 0 else (None, None)
            if pbar:
                pbar.update(1)
            ...
            if audio_pad.shape[0] > self.t_max:
                rich_logger.debug(
                    "Processing long audio file with optimization...")
                ...
                for t in range(self.t_center, audio.shape[0], self.t_center):
                    opt_ts.append(t -
                                  self.t_query +
                                  np.where(np.abs(audio_sum[t -
                                                            self.t_query: t +
                                                            self.t_query]) == np.abs(audio_sum[t -
                                                                                               self.t_query: t +
                                                                                               self.t_query]).min())[0][0])
            ...
            if pitch_guidance:
                ...
                pitch, pitchf = torch.tensor(pitch[:p_len], device=self.device).unsqueeze(
                    0).long(), torch.tensor(pitchf[:p_len], device=self.device).unsqueeze(0).float()
            ...
            if energy_use:
                ...
                try:
                    if not hasattr(self, "rms_extract"):
                        from ...lib.infer.extracting.rms import \
                            RMSEnergyExtractor
            ...
            for t in opt_ts:
                ...
                rich_logger.debug(
                    f"Processing audio chunk: {s} to {
                        t + self.t_pad2 + self.window}")
            ...
            if rms_mix_rate != 1:
                rich_logger.debug(
                    f"Applying RMS mixing (rate: {rms_mix_rate})")
            ...
            self.conversion_stats["average_time"] = (
                self.conversion_stats["total_time"] /
                self.conversion_stats["total_conversions"])

            rich_logger.debug(
                f"Pipeline conversion completed in {
                    elapsed_time:.2f}s")
    ...
    def get_conversion_stats(self) -> dict:
        ...
        return {
            ...
            "conversions_per_second": 1.0 /
            self.conversion_stats["average_time"] if self.conversion_stats["average_time"] > 0 else 0}
...
class EnhancedConfig:
    ...
    def __init__(self, device="cpu", is_half=False):
        ...
        rich_logger.debug(
            f"Enhanced config initialized - Device: {
